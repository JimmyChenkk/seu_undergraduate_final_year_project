...
针对目标域伪标签不可靠问题，本章提出多证据置信均衡伪标签方法。该方法在第三章模型基础上加入教师模型、强弱增强一致性、多证据语义融合和类别均衡伪标签学习。训练过程不依赖目标域标签，而是融合三类证据：由非平衡传输计划得到的传输语义分布、由 EMA 教师模型得到的预测语义分布，以及由源域类别原型得到的原型语义分布。只有当多种证据一致、置信度较高且传输语义熵较低时，目标样本才参与伪标签学习。方法整体结构如图 \ref{fig:ch4_cbtpu_pseudolabel} 所示。
...
本节围绕第三章、第四章和第五章提出的三类改良方法展开有效性验证。为突出各创新模块本身的作用，本节仅将所提方法与源域直接迁移方法（SourceOnly）进行对比：若不进行领域自适应，仅依靠源域有标签样本训练得到的模型在目标域上性能较低，而引入本文方法后目标域准确率、宏平均 F1 和平衡准确率明显提高，则说明对应改良机制能够缓解跨工况分布偏移。各结果均来自固定第一折划分下的三轮重复实验，数值为目标域测试集百分比形式，格式为“均值 $\pm$ 标准差”。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c c}
        ...
        方法 & 准确率 & 宏平均F1 & 平衡准确率 \\
        \hline
        SourceOnly & 55.88$\pm$7.45 & 50.76$\pm$7.75 & 55.73$\pm$7.49 \\
...
由表 \ref{tab:ch6_single_sourceonly_proposed_average} 可知，SourceOnly 的平均准确率仅为 55.88\%，说明 mode 1、mode 2 和 mode 5 之间存在明显跨工况分布偏移。TPU-\allowbreak{}Deep\allowbreak{}JDOT 将平均准确率提高至 77.49\%，较 SourceOnly 提升 21.61 个百分点；宏平均 F1 从 50.76\% 提升至 75.54\%，平衡准确率从 55.73\% 提升至 77.68\%。这表明，第三章方法不仅提高了整体分类正确率，也改善了多类别故障诊断中的均衡识别能力。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{c c c c}
        ...
        \hline
        $m_1\rightarrow m_2$ & 58.73$\pm$0.25 & 73.84$\pm$0.51 & 73.96$\pm$1.26 \\
        $m_1\rightarrow m_5$ & 68.87$\pm$1.91 & 78.07$\pm$4.87 & 79.69$\pm$0.99 \\
        $m_2\rightarrow m_1$ & 50.11$\pm$1.37 & 76.67$\pm$0.59 & 75.00$\pm$1.86 \\
        $m_2\rightarrow m_5$ & 59.03$\pm$1.23 & 78.07$\pm$0.57 & 78.59$\pm$3.05 \\
        $m_5\rightarrow m_1$ & 46.90$\pm$0.00 & 77.30$\pm$0.08 & 78.62$\pm$0.99 \\
        $m_5\rightarrow m_2$ & 51.62$\pm$2.74 & 81.01$\pm$0.98 & 82.30$\pm$0.51 \\
        \hline
    \end{tabular}
\end{table}

表 \ref{tab:ch6_single_sourceonly_proposed_scene} 进一步显示，TPU-\allowbreak{}Deep\allowbreak{}JDOT 在 6 个单源迁移任务上均明显高于 SourceOnly。其中，$m_5\rightarrow m_1$ 和 $m_5\rightarrow m_2$ 的提升分别达到 30.40 和 29.39 个百分点，说明当源目标工况之间存在较强偏移时，非平衡传输能够减少小批量中错误强制匹配，源域类别原型和时序统计代价也能够帮助模型保留故障类别结构。因此，第三章提出的时序原型非平衡联合分布对齐方法在单源无监督跨工况故障诊断中具有明确有效性。

\subsection{多证据置信均衡伪标签有效性}

第四章提出的 CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 在第三章方法基础上进一步利用目标域无标签样本。该方法通过 EMA 教师模型、传输语义、教师预测语义和源域原型语义共同筛选伪标签，并结合动态阈值、类别均衡和强弱增强一致性约束，使目标域训练样本的利用从单纯对齐扩展为高置信目标样本驱动。

从表 \ref{tab:ch6_single_sourceonly_proposed_average} 可以看出，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 的平均准确率达到 78.03\%，较 SourceOnly 提升 22.15 个百分点；宏平均 F1 和平衡准确率分别提升 25.27 和 22.48 个百分点。相较于 SourceOnly，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 在所有单源迁移任务上均取得明显更高的目标域准确率，尤其在 $m_5\rightarrow m_2$、$m_5\rightarrow m_1$、$m_2\rightarrow m_5$ 等任务中，目标域诊断性能提升较为充分。

与第三章方法相比，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 的额外增益相对温和，但其相对 SourceOnly 的提升具有稳定性。这说明多证据伪标签机制能够在已有对齐基础上继续利用目标域无标签信息，进一步强化目标域决策边界。需要注意的是，伪标签方法依赖初始预测质量，在 $m_2\rightarrow m_1$ 等个别场景中不一定优于 TPU-\allowbreak{}Deep\allowbreak{}JDOT；但从验证第四章创新方法是否相对 SourceOnly 有效的角度看，其总体和逐场景结果均支持该机制的有效性。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figs/ch6_single_source_heatmap.pdf}
    \caption{单源跨工况实验各方法逐场景准确率汇总热力图}
    \label{fig:single_source_heatmap}
\end{figure}

图 \ref{fig:single_source_heatmap} 展示了单源任务中不同方法在各迁移方向上的目标域准确率。SourceOnly 在多数任务上明显偏低，说明跨生产模式直接泛化能力不足；DeepJDOT、CoDATS 等领域自适应方法整体改善了目标域性能；TPU-\allowbreak{}Deep\allowbreak{}JDOT 和 CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 在多数迁移方向上进一步提高准确率，表明时序原型非平衡对齐和多证据伪标签机制能够提升单源跨工况诊断效果。
...
表 \ref{tab:single_average_results} 给出了单源方法平均性能。CoDATS 和 DeepJDOT 分别取得 75.69\% 和 74.29\% 的平均准确率，说明卷积式时间序列领域对抗表征和联合分布最优传输均能缓解跨工况退化。TPU-\allowbreak{}Deep\allowbreak{}JDOT 与 CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 分别达到 77.49\% 和 78.03\%，为无监督单源方法中的较优结果，但与目标域监督参考仍存在约 12 个百分点差距，说明目标域缺少标签仍是跨工况诊断的核心困难。
...
从表 \ref{tab:single_scene_accuracy} 看，TPU-\allowbreak{}Deep\allowbreak{}JDOT 和 CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 在 $m_1\rightarrow m_5$、$m_2\rightarrow m_5$、$m_5\rightarrow m_1$ 和 $m_5\rightarrow m_2$ 等任务中表现突出，说明当源目标类别结构能够被较可靠估计时，原型约束、时序统计代价和伪标签筛选可以共同改善目标域决策边界。但在 $m_1\rightarrow m_2$ 中，DeepJDOT 仍高于本文单源改良方法，说明目标域为分布差异较大的 $m_2$ 时，目标域类别结构估计仍可能成为限制因素。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figs/ch6_multi_source_heatmap.pdf}
...
单源实验的主线目标是：在目标域无标签条件下，缓解单一源域到目标域的工况分布偏移，并提高目标域故障诊断性能。实验结果表明，SourceOnly 的平均准确率仅为 55.88\%，而 TPU-\allowbreak{}Deep\allowbreak{}JDOT 和 CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 分别达到 77.49\% 和 78.03\%。混淆矩阵和域融合 t-SNE 也显示，本文单源方法能够增强目标域类别边界和源目标特征融合。因此，第三章和第四章围绕“更可靠的单源对齐”和“更可靠的目标域无标签样本利用”所设定的目标基本实现。
