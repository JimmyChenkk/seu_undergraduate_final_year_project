...
与 DeepJDOT 相比，TPU-\allowbreak{}Deep\allowbreak{}JDOT 的平均准确率提高到 77.49\%，提升约 3.20 个百分点，宏平均 F1 和平衡准确率也同步提高。这说明非平衡传输、源域类别原型、时序统计代价和源域监督对比学习能够整体改善单源跨工况对齐质量。尤其在 $m_1$ 与 $m_5$ 互迁、$m_5\rightarrow m_2$ 等相对可迁移场景中，TPU-\allowbreak{}Deep\allowbreak{}JDOT 对 DeepJDOT 的提升较明显，表明第三章引入的时序和类别结构约束有助于形成更稳定的目标域决策边界。
...
表 \ref{tab:single_scene_accuracy} 给出了核心单源方法在各迁移任务上的准确率。源域直接迁移方法在 $m_5\rightarrow m_1$ 和 $m_2\rightarrow m_1$ 等任务上性能较低，说明目标域为 $m_1$ 时直接迁移存在较强工况偏移。DeepJDOT 在所有单源任务上均高于源域直接迁移方法，说明联合分布最优传输能够缓解源目标分布差异。TPU-\allowbreak{}Deep\allowbreak{}JDOT 在多数任务中进一步提高准确率，但在 $m_1\rightarrow m_2$ 场景中低于 DeepJDOT，表明当目标域为分布差异较大的 $m_2$ 时，非平衡传输和原型约束仍可能受到目标域类别结构不稳定的影响。
...
逐场景结果进一步表明，TPU-\allowbreak{}Deep\allowbreak{}JDOT 的收益在 $m_1\rightarrow m_5$、$m_2\rightarrow m_1$、$m_5\rightarrow m_1$ 和 $m_5\rightarrow m_2$ 等任务中较为明显。这些场景中，源域直接迁移性能偏低或 DeepJDOT 对齐仍不充分，时序原型非平衡对齐能够提供更稳定的类别结构约束。但在 $m_1\rightarrow m_2$ 场景中，TPU-\allowbreak{}Deep\allowbreak{}JDOT 低于 DeepJDOT，说明目标域为分布差异较大的 $m_2$ 时，目标域类别结构不稳定仍可能削弱原型约束的效果。
...
图 \ref{fig:single_source_heatmap} 展示了不同迁移方向上的性能差异。目标域为 $m_2$ 的任务整体更困难，而以 $m_1$ 和 $m_5$ 互迁为代表的场景更容易从时序原型约束和伪标签机制中获得收益。
...
CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 的混淆矩阵主要用于观察多证据伪标签机制是否改善类别级召回率。若多数故障类别在对角线上保持较高响应，同时正常状态和少数响应相近故障仍出现混淆，则说明该方法能够利用高置信目标样本强化多数类别边界，但对低置信类别和早期预测偏差较大的类别仍不够充分。尤其在目标域为 $m_2$ 的任务中，需重点检查类别 0、3、9、15 和 21 等容易混淆类别的召回率变化，以判断伪标签筛选是否引入类别偏置。
...
多源实验用于验证第五章提出的类条件源域可靠性加权方法。实验包含两源域到单目标域和五源域到单目标域两类设置。两源实验主要考察 $m_1$、$m_2$ 和 $m_5$ 三个生产模式之间的小规模多源迁移；五源实验采用留一领域评估策略，用于观察源域数量增加后模型的性能边界和负迁移风险。比较方法包括源域直接迁移、CoDATS、WJDOT、CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 和目标域监督参考方法，其中目标域监督参考方法仅用于提供有标签目标域条件下的参考结果。
...
表 \ref{tab:two_source_scene_accuracy} 表明，在两源任务中 CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 在三个目标域上均取得了所比较无监督方法中的较好结果。对于 $m_2+m_5\rightarrow m_1$，WJDOT 高于 CoDATS，说明最优传输式多源加权在该目标域上能够带来收益；CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 在此基础上继续提升，说明类条件源域可靠性矩阵可以进一步细化源域贡献。对于 $m_1+m_5\rightarrow m_2$，$m_2$ 作为目标域时跨工况差异较大，但 CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 仍保持高于 CoDATS 和 WJDOT 的平均准确率，说明教师先验约束和类条件可靠性加权对困难目标域具有一定稳定作用。
