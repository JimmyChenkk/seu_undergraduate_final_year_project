...
\frontmatter
\begin{abstract}[zh]
在田纳西伊斯曼过程的跨生产模式诊断中，同一类故障在不同模式下可能呈现不同变量结构。源模式中训练得到的诊断模型，一旦直接用于目标模式，往往会同时面对分布偏移和目标标签缺失。围绕这一设定，本文以田纳西伊斯曼过程领域自适应数据集为研究对象，分别研究单源域到单目标域和多源域到单目标域两类跨工况故障诊断任务。

在单源场景中，本文以 DeepJDOT 为起点，提出时序原型非平衡联合分布对齐方法。该方法利用非平衡传输削弱小批量训练中的错误强制匹配，并将源域类别原型、时间序列统计代价和监督对比学习纳入联合对齐过程，以保持故障类别结构。随后，针对目标域伪标签可靠性不足的问题，本文利用传输语义、教师预测语义和源域原型语义分布筛选目标样本，形成多证据置信均衡伪标签方法，并通过动态阈值、类别均衡和强弱增强一致性约束抑制伪标签噪声。在多源场景中，本文从原型距离、类条件传输代价、目标预测熵和源域类别性能等方面构建源域--故障类别可靠性矩阵，支持更细粒度的多源选择性迁移。实验结果表明，上述方法能够提升跨工况故障诊断的目标域性能，并为分析不同源域和故障类别的迁移贡献提供依据。
\keywords{工业过程故障诊断,领域自适应,最优传输,伪标签学习,多源迁移}
\end{abstract}

\begin{abstract}[en]
In cross-mode diagnosis on the Tennessee Eastman process benchmark, the same fault category may appear with different process patterns under different production modes. A diagnostic model trained on a source mode therefore faces both distribution shift and unavailable target labels when it is deployed in a target mode. This thesis studies domain adaptation methods for this setting, covering both single-source to single-target and multi-source to single-target tasks.

For the single-source setting, the work starts from DeepJDOT and develops a temporal-prototype unbalanced joint distribution alignment method. Unbalanced transport reduces unreliable forced matching in mini-batch alignment, while source-class prototypes, temporal costs, and supervised contrastive learning help preserve fault-category structures. A confidence-balanced pseudo-labeling method is then introduced to select target samples through transport semantics, teacher predictions, and source-prototype semantics. For the multi-source setting, a class-conditional source reliability weighting method constructs a source-fault reliability matrix from prototype distance, class-conditional transport cost, target prediction entropy, and source-class performance. Experimental results show improved target-domain diagnosis performance and provide evidence for the transfer contribution of different source domains and fault categories.
...
田纳西伊斯曼过程这类多模式流程工业数据中，跨工况诊断的难点并不只是“换一个工况”。不同生产模式会改变连续变量之间的相关结构，正常样本和故障样本在统计分布上都可能随之移动；同一故障在时间窗口中的响应，也可能不再保持源工况下的形态。若仍把源工况训练得到的诊断边界直接用于目标工况，模型面对的往往不是一个简单的分类任务，而是分布偏移、类别保持和目标标签缺失同时存在的问题。

从任务本身看，工业过程故障诊断通常包含两步：先判断系统是否已经偏离正常状态，再进一步区分故障类型。过程变量数量多、变量之间存在耦合关系，故障发生后改变的也往往不是某一个孤立变量，而是多个变量之间的联动模式。因此，数据驱动方法能够利用历史过程数据学习故障特征，在复杂系统中比完全依赖精确机理模型更易落地。近年来，深度学习方法凭借端到端表征能力，已经被用于旋转机械、化工过程和其他工业故障诊断任务\cite{lei2020applications,wang2025comprehensive}。

不过，监督诊断模型通常默认训练集和测试集来自同一分布。在跨生产模式场景下，这一假设很容易被破坏：源模式中清晰的故障簇，到了目标模式可能被拉伸、平移，甚至与其他类别发生局部重叠。与此同时，目标工况的故障标签往往难以大量获取，尤其是低频故障和危险工况，不能指望通过充分标注重新训练一个目标域模型。本文关注的问题正是：如何利用源工况有标签样本，同时吸收目标工况无标签数据中的分布信息，使模型在目标模式下仍能维持可用的诊断能力。

领域自适应为这一问题提供了合适的建模语言。它不要求目标域提供标签，而是利用源域有标签数据和目标域无标签数据，缩小二者在特征空间或联合分布上的差异\cite{pan2009survey,wang2025comprehensive}。在工业过程故障诊断中，不同生产模式可视为不同领域，故障类别空间通常保持一致；这使得无监督领域自适应能够直接对应“源工况有标签、目标工况无标签”的实际约束。

本文选取田纳西伊斯曼过程领域自适应数据集作为研究对象。该数据集是化工过程控制与故障诊断研究中的常用基准，不同生产模式具有不同的数据分布；已有基准研究也表明，模式之间在变量相关结构和 Wasserstein 距离上存在差异\cite{montesuma2024benchmark}。围绕这一数据集，本文分别研究单源域到单目标域、以及多源域到单目标域两类任务，并在最优传输、时序原型、置信伪标签和类条件源域可靠性四条线索上展开方法设计。

\section{国内外研究现状}

\subsection{工业过程故障诊断研究现状}

工业过程故障诊断方法大致可以分为基于机理模型、基于信号处理和基于数据驱动三类。机理模型依赖系统方程、残差分析或状态估计，解释性较强，但在变量众多、控制回路复杂的流程工业中，精确建模和参数辨识成本较高。过程监测领域的综述已经指出，数据驱动建模逐渐成为复杂流程工业监测的重要路线，其优势在于能够直接利用历史运行数据刻画多变量过程状态\cite{qin2012survey,ge2017review}。

基于信号处理的方法通常先提取时域、频域或时频域特征，再结合分类器完成故障识别。这一路线在旋转机械和传感器信号分析中较常见，但特征设计往往依赖人工经验，对设备和工况变化较敏感。随着工业数据规模增大，支持向量机、随机森林、人工神经网络和卷积网络等数据驱动方法被更多用于化工过程故障检测与诊断\cite{lei2020applications,taqvi2021review,wang2025comprehensive}。

对于田纳西伊斯曼过程这类多变量时间序列，故障并不总是表现为单个变量的瞬时异常。很多时候，真正能区分类别的是多个变量在若干采样时刻内形成的动态响应。全卷积网络、循环网络和注意力结构因此被引入时间序列故障诊断，用于捕捉局部波动、较长时间依赖和变量间关系。相关研究也表明，深度过程故障诊断在跨工况泛化、类别不均衡和目标域标签不足方面仍存在明显挑战\cite{yu2023challenges}。

\subsection{迁移学习研究现状}

迁移学习试图回答一个实际问题：当目标场景的数据不足时，能否把已有场景中的知识迁移过来。根据迁移对象不同，可分为样本迁移、特征迁移、模型迁移和关系迁移等类型；根据源任务与目标任务是否相同，又可讨论同构迁移、异构迁移、领域自适应等问题\cite{pan2009survey,li2022perspective}。

在故障诊断中，迁移学习常用于跨设备、跨负载或跨工况场景。例如，源工况故障样本相对充分，而目标工况只有正常数据或少量故障数据时，直接训练目标域模型并不现实。深度迁移学习将神经网络表征学习能力与知识迁移结合起来，使模型可以在源域学习可迁移特征，再服务于目标域诊断任务\cite{li2022perspective}。

这一路线也有边界。源域和目标域差异过大时，迁移未必带来收益，甚至可能造成负迁移。对工业过程而言，同一故障类别在不同生产模式下的特征中心、变量相关结构和动态响应都可能变化。因此，迁移方法不能只追求“更多源域信息”，还需要判断哪些样本、哪些类别或哪些源域对目标工况是可靠的。

\subsection{领域自适应研究现状}

领域自适应是迁移学习中与本文最接近的一类问题。典型设定下，源域拥有标签，目标域没有标签或只有很少标签，二者类别空间相同但样本分布不同。本文采用无监督领域自适应设定，即训练过程中不使用目标域标签，只利用目标域无标签样本参与对齐。

现有领域自适应方法中，基于度量学习的路线通过最大均值差异、相关对齐、条件分布差异或 Wasserstein 距离等度量缩小域间差异。最优传输方法的优势在于可以保留样本级匹配关系，并从分布几何角度描述源域样本如何对应到目标域样本\cite{courty2017otda}。DeepJDOT 进一步把最优传输放入深度特征与标签预测的联合空间，在缩小域间差异的同时保留一定的类别判别信息\cite{damodaran2018deepjdot}。

另一类常用方法是基于对抗学习的特征对齐。领域对抗网络通过领域判别器迫使特征提取器学习源域和目标域难以区分的表征；面向时间序列数据，CoDATS 采用卷积式时间序列特征提取器并结合领域对抗学习，能够处理传感器序列的跨域迁移任务\cite{wilson2020codats}。在多源场景下，WJDOT 通过逐源联合分布最优传输和源域权重学习，避免把所有源域简单混合为一个大源域\cite{turrisi2022wjdot}。

针对田纳西伊斯曼过程领域自适应数据集的基准研究表明，不同生产模式确实会引起明显分布偏移，基于最优传输的方法在该基准上整体优于部分基于最大均值差异、H-distance 和对抗学习的基线\cite{montesuma2024benchmark}。但这些结论也留下了进一步的问题：DeepJDOT 类方法仍可能受小批量强制匹配影响；伪标签若只依赖分类器置信度，容易把早期错误放大；多源方法如果只给每个源域一个整体权重，也难以刻画“某个源域只对部分类别可靠”的情况。

\section{国内外研究现状总结}

上述研究说明，工业故障诊断已经从机理建模、人工特征设计，逐步进入深度表征和迁移诊断并行发展的阶段。对本文任务来说，关键并不只是采用更强的分类器，而是要在目标域无标签的约束下处理三类具体困难：源目标样本如何对齐，目标样本是否可信，以及多个源域该如何被选择性利用。

单源场景中，联合分布最优传输能够同时利用特征距离和标签预测信息，但普通平衡传输在小批量内可能强迫不合适的源目标样本建立匹配。目标域伪标签阶段又会遇到另一层问题：分类器的高置信度并不总等于正确，特别是在目标域边界尚未稳定时，错误伪标签可能沿训练过程持续传播。多源场景中，源域数量增加并不自动意味着性能提升，因为同一源域对不同故障类别的可迁移性可能不同。

基于这些观察，本文形成两条互相关联的研究路线。单源路线以 DeepJDOT 为基础，先通过时序原型非平衡联合分布对齐缓解错误匹配并保持类别结构，再通过多证据置信均衡伪标签学习提高目标样本使用质量。多源路线则在 CoDATS 和 WJDOT 的基础上，引入类条件源域可靠性加权，使源域选择从整体层面细化到故障类别层面。

\section{核心问题与主要创新}

本文研究目标是在目标工况无标签条件下，利用源工况有标签数据完成跨生产模式故障诊断。结合田纳西伊斯曼过程数据特点和现有方法局限，本文主要围绕以下三个问题展开。

第一，单源跨工况诊断中的类别结构保持问题。DeepJDOT 能够在特征和标签预测联合空间中对齐源域与目标域，但普通平衡传输在小批量类别组成不一致时容易产生错误强制匹配；同时，原始传输代价没有显式利用工业时间序列中的故障动态和类别原型。本文提出时序原型非平衡联合分布对齐方法，在 DeepJDOT 基础上加入非平衡最优传输、源域类别原型、时间序列统计代价和源域监督对比学习。

第二，目标域伪标签可靠性问题。目标域标签不可用时，伪标签是利用目标样本的重要入口，但单一分类器置信度容易受早期错误和类别偏置影响。本文提出多证据置信均衡伪标签方法，从传输计划、教师模型预测和源域原型距离三类证据中筛选目标样本，并配合动态置信阈值、类别均衡和强弱增强一致性约束，降低错误伪标签的传播风险。

第三，多源跨工况诊断中的源域负迁移问题。多源任务虽然提供了更多有标签样本，但不同源域与目标域之间的距离并不一致，同一源域对不同故障类别的可靠性也可能不同。本文提出类条件源域可靠性加权方法，将 CoDATS 的卷积式时间序列表征、WJDOT 的逐源联合分布传输和源域--故障类别可靠性矩阵结合起来，使多源迁移可以在类别层面选择更可靠的源域信息。
...
本文技术路线围绕“数据整理—基础模型—单源改进—多源改进—实验验证”展开。数据层面，本文整理田纳西伊斯曼过程领域自适应数据集，将不同生产模式视为不同领域；样本形式为多变量时间序列，标签包括正常状态和多类故障状态。实验中目标域标签只用于最终评估，不参与训练、参数选择或模型选择。

模型层面，单源任务以 DeepJDOT 为基础，利用深度特征与标签预测的联合最优传输进行源目标对齐；多源任务以 WJDOT 为基础，通过逐源传输和源域权重刻画多源迁移关系。考虑到过程数据具有时间序列结构，本文在多源方法中引入卷积式时间序列特征提取和领域对抗约束。

方法层面，第三章首先处理 DeepJDOT 中平衡传输和故障结构保持不足的问题，提出时序原型非平衡联合分布对齐方法。第四章在此基础上进一步处理目标域伪标签可靠性，提出多证据置信均衡伪标签学习方法。第五章面向多源诊断，引入类条件源域可靠性矩阵，使不同源域对不同故障类别的贡献能够被区分和利用。

实验层面，本文分别设置单源和多源跨工况任务，并使用准确率、宏平均 F1、平衡准确率、混淆矩阵和跨域特征可视化等指标进行验证。除整体性能外，实验还关注关键机制、典型场景和源域可靠性差异，避免仅依赖单一任务结果判断方法有效性。

\section{论文结构}

本文共分为七章，各章之间的关系如图 \ref{fig:ch1_thesis_structure} 所示。

第一章说明跨工况故障诊断的研究背景、相关研究进展和本文的三个具体问题。第二章介绍后续方法所需的联合分布最优传输、DeepJDOT、WJDOT 和卷积式时间序列领域自适应基础。第三章提出时序原型非平衡联合分布对齐方法。第四章在第三章基础上引入多证据置信均衡伪标签学习。第五章面向多源场景提出类条件源域可靠性加权方法。第六章给出田纳西伊斯曼过程数据集上的实验设计、性能比较和机制分析。第七章总结三条方法链的作用边界，并提出可继续验证的后续实验方向。
...
\begin{figure}[!htbp]
    ...
    \noindent\makebox[\linewidth][c]{%
        \resizebox{0.96\linewidth}{!}{\ignorespaces\input{figs/ch1_thesis_structure.tikz.tex}\unskip}%
    }
    \caption{论文整体框架}
...
跨工况诊断首先要面对分布不一致的问题。以田纳西伊斯曼过程为例，不同生产模式下的连续变量会发生统计迁移，源工况中学习到的分类边界未必适用于目标工况。由于目标模式故障标签难以大量获取，本文将这一任务建模为无监督领域自适应问题：源域提供有标签样本，目标域只提供无标签样本，训练过程不使用目标域标签。
...
在这一设定下，最优传输的价值在于给出样本级的跨域匹配关系，而不只是比较两个分布的整体统计量\cite{courty2017otda}。对多变量时间序列而言，特征提取器还必须保留故障动态响应，因此本章同时介绍卷积式时间序列特征提取和领域对抗学习\cite{wang2017time,fawaz2019deep}。

本章只保留后续章节真正需要的基础方法：联合分布最优传输说明源目标样本如何匹配，DeepJDOT 给出单源联合对齐形式，WJDOT 给出多源源域加权思路，CoDATS 则提供时间序列表征和领域对抗对齐框架。

\section{基于联合分布最优传输的领域自适应方法}

最优传输关注的问题是：若要把源分布中的质量移动到目标分布，最低总代价是多少。这个表述看似抽象，但在领域自适应中非常直接，它把“源域哪个样本对应目标域哪个样本”写成了一个可优化的传输计划。设源域样本集合为 $\{x_i^s\}_{i=1}^{n_s}$，目标域样本集合为 $\{x_j^t\}_{j=1}^{n_t}$，二者之间的代价矩阵为 $C \in \mathbb{R}^{n_s \times n_t}$，其中 $C_{ij}$ 表示源样本 $i$ 与目标样本 $j$ 的匹配代价。最优传输问题可写为
\begin{equation}
    \gamma^\ast
    =
...
WJDOT 的出发点是承认多个源域并不等价。对田纳西伊斯曼过程而言，不同生产模式与目标模式的接近程度不同，逐源权重比把所有源域直接合并更合理。

不过，源域整体权重只能回答“哪个源域更重要”，不能回答“这个源域对哪一类故障更可靠”。一个源模式可能对正常类和部分故障类较接近，却在其他故障类上偏移明显。因此，第五章将在 WJDOT 基础上引入类条件源域可靠性矩阵，把加权粒度推进到源域--故障类别层面。
...
其次，WJDOT 将联合分布最优传输放入多源设定，通过源域权重区分不同源域对目标域的贡献。这个设定适合田纳西伊斯曼过程的多生产模式任务，但源域整体权重仍不足以表达类别层面的可靠性差异。
...
再次，目标域无标签条件下的模型选择仍然困难。本文训练过程不使用目标域评估标签，只能依靠源域性能、目标预测分布、教师先验等间接信号判断模型状态。当目标域预测发生类别坍缩或局部过置信时，这些无标签指标可能与真实目标域性能不完全一致。
