...
本节围绕第三章、第四章和第五章提出的三条方法主线进行有效性验证。为避免将机制验证写成完整基线汇总，本节只比较相邻方法之间的增量关系：TPU\_DPJDOT 相对 DeepJDOT，CBTPU\_DPJDOT 相对 TPU\_DPJDOT，CA\_CCSR\_WJDOT 相对 WJDOT。完整基线对比统一放在 6.4 节展开。

正文结果均采用统一的数据划分和随机性控制。表中“均值 $\pm$ 标准差”表示不同迁移任务或目标域之间的均值与场景标准差；其他随机性条件下的补充实验仅作为附录候选，不在正文中展开讨论。

\subsection{时序原型非平衡联合分布对齐有效性}

第三章方法的目标是缓解 DeepJDOT 在小批量联合分布最优传输中可能出现的错误强制匹配，并通过源域类别原型、时序统计代价和源域监督对比约束保持故障类别结构。因此，本小节只比较 DeepJDOT 与 TPU\_DPJDOT。

在 6 个单源迁移任务上，DeepJDOT 的平均准确率、宏平均 F1 和平衡准确率分别为 68.88\%、66.04\% 和 69.08\%；TPU\_DPJDOT 分别达到 80.54\%、78.82\% 和 80.64\%。相较 DeepJDOT，TPU\_DPJDOT 的平均准确率提升 11.66 个百分点，宏平均 F1 提升 12.78 个百分点，平衡准确率提升 11.56 个百分点，并且在全部 6 个单源迁移方向上均取得更高目标域准确率。

本小节核心结论是：时序原型非平衡联合分布对齐能够稳定增强 DeepJDOT 的跨工况类别结构保持能力。

\subsection{多证据置信均衡伪标签有效性}

第四章方法的目标是在 TPU\_DPJDOT 已完成较可靠源目标对齐的基础上，进一步利用目标域无标签样本。CBTPU\_DPJDOT 通过冻结 TPU\_DPJDOT 教师锚点、多证据伪标签筛选、类别均衡和强弱增强一致性约束，减少错误伪标签对目标域决策边界的干扰。因此，本小节只比较 CBTPU\_DPJDOT 与 TPU\_DPJDOT。

CBTPU\_DPJDOT 的平均准确率、宏平均 F1 和平衡准确率分别为 82.02\%、80.73\% 和 82.12\%，相较 TPU\_DPJDOT 继续提升 1.48、1.91 和 1.48 个百分点。逐任务结果中，CBTPU\_DPJDOT 在 6 个单源迁移方向上均高于 TPU\_DPJDOT，其中 $m_1\rightarrow m_2$ 和 $m_5\rightarrow m_1$ 的额外提升分别达到 3.00 和 3.79 个百分点。

本小节核心结论是：多证据置信均衡伪标签机制能够在已有单源对齐基础上继续带来小幅但一致的目标域收益。

\subsection{类条件源域可靠性加权有效性}

第五章方法的目标是解决 WJDOT 全局源域权重粒度较粗的问题。CA\_CCSR\_WJDOT 将源域加权细化到源域--故障类别层面，并结合 CoDATS 风格时间序列表征和教师先验融合，使模型能够为不同故障类别选择更可靠的源域信息。因此，本小节只比较 CA\_CCSR\_WJDOT 与 WJDOT。

在五源留一域设置中，WJDOT 的平均准确率、宏平均 F1 和平衡准确率分别为 78.37\%、75.70\% 和 78.47\%；CA\_CCSR\_WJDOT 分别达到 85.50\%、84.30\% 和 85.51\%。相较 WJDOT，CA\_CCSR\_WJDOT 的平均准确率提升 7.13 个百分点，宏平均 F1 提升 8.60 个百分点，平衡准确率提升 7.04 个百分点，并在 6/6 个目标域上取得无监督方法中的最高准确率。
...
本小节汇总单源跨工况实验的完整基线。参与比较的方法包括 SourceOnly、DSAN、CDAN-TS、CoDATS、DeepJDOT、TPU\_DPJDOT、CBTPU\_DPJDOT 和 Target-ref。其中 Target-ref 使用目标域训练部分标签进行监督训练，仅作为有标签参考，不参与无监督方法公平排序。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c c}
        ...
        TPU\_DPJDOT & 80.54$\pm$2.20 & 78.82$\pm$2.18 & 80.64$\pm$2.28 \\
        CBTPU\_DPJDOT & 82.02$\pm$1.64 & 80.73$\pm$1.54 & 82.12$\pm$1.71 \\
...
表 \ref{tab:single_average_results}、图 \ref{fig:single_source_accuracy_bar} 和图 \ref{fig:single_source_heatmap} 表明，SourceOnly 的平均准确率仅为 55.88\%，说明单一生产模式监督训练难以直接泛化到目标模式。传统领域自适应方法中，CoDATS 的平均准确率为 74.30\%，是较强的时间序列对抗适配基线；DeepJDOT 的平均准确率为 68.88\%，说明普通联合分布最优传输在部分跨工况方向上仍受到错误匹配影响。TPU\_DPJDOT 和 CBTPU\_DPJDOT 分别达到 80.54\% 和 82.02\%，其中 CBTPU\_DPJDOT 在全部 6 个单源迁移方向上取得无监督方法中的最高准确率。

本小节核心结论是：单源主线呈现 DeepJDOT $\rightarrow$ TPU\_DPJDOT $\rightarrow$ CBTPU\_DPJDOT 的稳定递进关系，且最终方法整体优于传统单源基线。

\subsection{多源完整基线对比与准确率汇总}

本小节汇总五源留一域设置下的完整基线。参与比较的方法包括 SourceOnly、CoDATS、WJDOT、CA\_CCSR\_WJDOT 和 Target-ref。两源结果用于补充检验小规模多源情形，正文综合对比以覆盖 6 个目标生产模式的五源主设置为准。
...
表 \ref{tab:multi_average_results}、图 \ref{fig:multi_source_accuracy_bar} 和图 \ref{fig:multi_source_heatmap} 表明，CoDATS 相比 SourceOnly 取得明显提升，说明卷积式时间序列表征和领域对抗约束能够有效利用多源信息。WJDOT 的平均准确率为 78.37\%，低于 CoDATS，说明仅依赖全局源域权重时，逐源联合分布传输仍可能受到源域差异和类别级负迁移影响。CA\_CCSR\_WJDOT 达到 85.50$\pm$2.50\%，高于 CoDATS 和 WJDOT，并在 6/6 个目标域上取得无监督方法第一。

本小节核心结论是：CA\_CCSR\_WJDOT 在五源留一域设置中整体优于 CoDATS 和 WJDOT。

\section{代表性场景可视化与机制分析}

\subsection{单源代表场景分析}

为避免在正文中堆叠所有任务的可视化结果，本小节选取较困难的单源任务 $m_5\rightarrow m_2$ 作为代表场景，分别展示目标域混淆矩阵和域融合特征可视化结果。该任务用于观察 DeepJDOT、TPU\_DPJDOT 和 CBTPU\_DPJDOT 在类别边界和源目标特征融合上的递进变化。
...
图 \ref{fig:single_confusion_matrix_grid} 和图 \ref{fig:single_tsne_domain_grid} 显示，SourceOnly 的类别错误分布较分散，源域和目标域特征也存在明显分离；DeepJDOT 能够拉近部分源目标样本，但仍存在类别混淆；TPU\_DPJDOT 通过非平衡传输和原型约束增强对角线响应；CBTPU\_DPJDOT 进一步利用高置信目标样本，使目标域样本更集中地进入可判别区域。
...
图 \ref{fig:multi_confusion_matrix_grid} 和图 \ref{fig:multi_tsne_domain_grid} 表明，SourceOnly 和 WJDOT 在目标域 $m_2$ 上仍存在较多类别混淆，说明简单合并源域或仅学习全局源域权重都不足以完全避免负迁移。CoDATS 通过时间序列表征和领域对抗约束改善整体识别能力；CA\_CCSR\_WJDOT 则进一步使目标域样本靠近对其类别更可靠的源域特征区域，与五源平均性能提升相一致。

本小节核心结论是：多源代表场景说明 CA\_CCSR\_WJDOT 的优势不是无差别混合所有源域，而是更有选择地利用可靠源域信息。

\subsection{类条件源域权重机制分析}

为使机制解释服务于结果，本小节仅保留一张源域--类别权重热力图，用于说明 CA\_CCSR\_WJDOT 为什么能够在五源设置中优于 WJDOT。图 \ref{fig:class_source_alpha_heatmap} 展示了五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中，不同源域对不同故障类别的类条件权重。
...
图 \ref{fig:class_source_alpha_heatmap} 反映出，不同故障类别并不总是依赖同一个源域：部分类别更偏向 $m_1$ 或 $m_5$，部分类别则更多利用 $m_4$ 或 $m_6$。这说明 CA\_CCSR\_WJDOT 的改进不是简单增加一个全局源域权重，而是在类别层面对源域贡献进行重新分配，从而解释了其相对 WJDOT 的平均性能提升和 6/6 目标域第一结果。

本小节核心结论是：源域--类别权重矩阵提供了对多源结果的必要解释，但正文只需保留该代表性机制图，其他诊断曲线和中间状态更适合作为附录候选。

\subsection{主线目标达成分析}

综合 6.3 节至 6.5 节结果，本文三条方法主线分别对应单源可靠对齐、目标域无标签样本利用和多源选择性迁移。单源实验中，TPU\_DPJDOT 相比 DeepJDOT 在平均准确率、宏平均 F1 和平衡准确率上均取得明显提升，说明时序原型、非平衡传输和时序统计代价能够缓解普通联合分布最优传输中的错误匹配；CBTPU\_DPJDOT 在此基础上继续获得一致增益，说明冻结教师锚点和多证据置信均衡伪标签机制能够进一步利用目标域无标签样本。单源代表场景中的混淆矩阵和域融合特征可视化也与该趋势一致。

多源实验中，CA\_CCSR\_WJDOT 在五源留一域完整基线中取得最高无监督平均准确率，并在 6 个目标域上均达到无监督方法第一。与 WJDOT 相比，该结果说明全局源域权重不足以刻画不同故障类别对源域的差异化需求，而类条件源域可靠性矩阵能够提供更细粒度的选择性迁移能力。源域--类别权重热力图进一步说明，不同故障类别确实依赖不同源域信息，从而为多源方法的性能提升提供了可解释支撑。
...
本章围绕田纳西伊斯曼过程领域自适应数据集，在统一实验协议下对单源和多源跨工况故障诊断方法进行了实验验证。单源实验表明，TPU\_DPJDOT 相比 DeepJDOT 明显提升平均准确率、宏平均 F1 和平衡准确率，验证了时序原型非平衡联合分布对齐的有效性；CBTPU\_DPJDOT 相比 TPU\_DPJDOT 进一步取得一致增益，验证了冻结教师锚点和多证据置信均衡伪标签机制的作用。

多源实验表明，CA\_CCSR\_WJDOT 在五源留一域设置中达到 85.50$\pm$2.50\% 的平均准确率，并在 6/6 个目标域上取得无监督方法第一，整体优于 CoDATS 和 WJDOT。该结果说明，类条件源域可靠性加权能够缓解 WJDOT 全局源域权重粒度过粗的问题，并提升多源选择性迁移能力。
