...
本文共分为七章，各章之间的关系如图 \ref{fig:ch1_thesis_structure} 所示。前两章先把跨工况诊断问题和后续用到的领域自适应模型讲清楚；第三章至第五章分别对应单源对齐、单源伪标签和多源可靠性三条方法线；第六章再把这些方法放到统一的 TEP 迁移任务中比较；第七章回到方法边界和后续实验设计。
...
第三章和第四章都处理单源到单目标任务，但分工不同。第三章的 TPU-DeepJDOT 先解决源目标样本如何对齐，重点放在非平衡传输、源域类别原型和时间序列统计代价上；第四章的 CBTPU-DeepJDOT 则接着处理目标样本能不能参与训练的问题，通过传输语义、教师预测语义和源域原型语义筛掉风险较高的伪标签。

第五章转向多源到单目标任务，提出 CA-CCSR-WJDOT 方法。与只学习一个源域整体权重不同，该章把可靠性拆到源域--故障类别层面，并把逐源联合传输、领域对抗约束和教师先验融合放在同一训练框架中。
...
本章的改动围绕这两个细节展开。首先，将平衡传输替换为非平衡小批量传输，允许低可靠匹配质量被削减；其次，维护源域类别原型，用目标样本到各源类原型的相对距离修正传输代价；同时，从原始过程窗口中提取均值、标准差和一阶差分统计量，补充深度特征中可能被压缩掉的时序信息。训练前期还加入源域监督对比学习，使源域特征先形成较清楚的类内聚集和类间分离。本章方法不使用目标域标签，目标域标签只在最终实验评价阶段使用。方法整体结构如图 \ref{fig:ch3_tpu_architecture} 所示。
...
本章采用更保守的做法：目标样本能否进入伪标签集合，不由学生分类器单独决定。教师模型由同一迁移场景下训练得到的 TPU-DeepJDOT 阶段模型初始化，并在后续训练中保持固定；学生模型同样从该阶段模型出发，只在多证据伪标签目标下做小幅微调。筛选时同时使用三类语义分布：非平衡传输计划给出的传输语义、固定教师模型给出的预测语义，以及源域原型给出的原型语义。只有当这些证据在类别判断上相互接近，且传输语义熵较低时，目标样本才被纳入伪标签学习。方法整体结构如图 \ref{fig:ch4_cbtpu_pseudolabel} 所示。
...
本章因此把源域权重拆到故障类别层面。方法沿用 CoDATS 的时间序列表征和 WJDOT 的逐源传输思路，但不只学习一个源域整体权重，而是在每个源域与目标域之间分别统计类条件传输代价，再结合源目标原型距离、目标预测熵和源域类别召回误差，形成源域--故障类别可靠性矩阵。训练和模型选择过程中不使用目标域标签，目标域标签仅用于最终评价。方法整体结构如图 \ref{fig:ch5_CA_CCSR_WJDOT} 所示。
