...
本文多源实验包含两类任务。第一类为模式1、模式2和模式5之间的两源域到单目标域任务，用于考察小规模多源条件下的迁移效果。该设置与单源双向迁移场景保持一致，只是每次选择两个模式作为源域，剩余一个模式作为目标域。具体任务如表 \ref{tab:two_source_tasks} 所示。

\begin{table}[htbp]
    \centering
    \caption{生产模式1、生产模式2和生产模式5之间的两源域到单目标域实验场景}
...
第二类为 6 个生产模式之间的五源域到单目标域任务。该设置采用留一域策略，即每次选择其中 1 个生产模式作为目标域，其余 5 个生产模式作为源域。该实验能够更全面地评估多源领域自适应方法在不同目标生产模式上的适用性，也更适合分析类条件源域可靠性矩阵是否能够刻画不同源域对不同故障类别的贡献。具体任务如表 \ref{tab:five_source_tasks} 所示。
...
本节围绕第三章、第四章和第五章提出的三条方法主线进行有效性验证。为避免将机制验证写成完整基线汇总，本节只比较相邻方法之间的增量关系：TPU-\allowbreak{}Deep\allowbreak{}JDOT 相对 DeepJDOT，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 相对 TPU-\allowbreak{}Deep\allowbreak{}JDOT，CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 相对 WJDOT。完整基线对比统一放在 6.4 节展开。

正文结果均采用统一的数据划分和随机性控制。表中“均值 $\pm$ 标准差”表示不同迁移任务或目标域之间的均值与场景标准差；其他随机性条件下的补充实验仅作为附录候选，不在正文中展开讨论。

\subsection{时序原型非平衡联合分布对齐有效性}

第三章方法的目标是缓解 DeepJDOT 在小批量联合分布最优传输中可能出现的错误强制匹配，并通过源域类别原型、时序统计代价和源域监督对比约束保持故障类别结构。因此，本小节只比较 DeepJDOT 与 TPU-\allowbreak{}Deep\allowbreak{}JDOT。

在 6 个单源迁移任务上，DeepJDOT 的平均准确率、宏平均 F1 和平衡准确率分别为 68.88\%、66.04\% 和 69.08\%；TPU-\allowbreak{}Deep\allowbreak{}JDOT 分别达到 80.54\%、78.82\% 和 80.64\%。相较 DeepJDOT，TPU-\allowbreak{}Deep\allowbreak{}JDOT 的平均准确率提升 11.66 个百分点，宏平均 F1 提升 12.78 个百分点，平衡准确率提升 11.56 个百分点，并且在全部 6 个单源迁移方向上均取得更高目标域准确率。

本小节核心结论是：时序原型非平衡联合分布对齐能够稳定增强 DeepJDOT 的跨工况类别结构保持能力。

\subsection{多证据置信均衡伪标签有效性}

第四章方法的目标是在 TPU-\allowbreak{}Deep\allowbreak{}JDOT 已完成较可靠源目标对齐的基础上，进一步利用目标域无标签样本。CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 通过冻结 TPU 教师锚点、多证据伪标签筛选、类别均衡和强弱增强一致性约束，减少错误伪标签对目标域决策边界的干扰。因此，本小节只比较 CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 与 TPU-\allowbreak{}Deep\allowbreak{}JDOT。

CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 的平均准确率、宏平均 F1 和平衡准确率分别为 82.02\%、80.73\% 和 82.12\%，相较 TPU-\allowbreak{}Deep\allowbreak{}JDOT 继续提升 1.48、1.91 和 1.48 个百分点。逐任务结果中，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 在 6 个单源迁移方向上均高于 TPU-\allowbreak{}Deep\allowbreak{}JDOT，其中 $m_1\rightarrow m_2$ 和 $m_5\rightarrow m_1$ 的额外提升分别达到 3.00 和 3.79 个百分点。

本小节核心结论是：多证据置信均衡伪标签机制能够在已有单源对齐基础上继续带来小幅但一致的目标域收益。

\subsection{类条件源域可靠性加权有效性}

第五章方法的目标是解决 WJDOT 全局源域权重粒度较粗的问题。CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 将源域加权细化到源域--故障类别层面，并结合 CoDATS 风格时间序列表征和教师先验融合，使模型能够为不同故障类别选择更可靠的源域信息。因此，本小节只比较 CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 与 WJDOT。

在五源留一域设置中，WJDOT 的平均准确率、宏平均 F1 和平衡准确率分别为 78.37\%、75.70\% 和 78.47\%；CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 分别达到 85.50\%、84.30\% 和 85.51\%。相较 WJDOT，CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 的平均准确率提升 7.13 个百分点，宏平均 F1 提升 8.60 个百分点，平衡准确率提升 7.04 个百分点，并在 6/6 个目标域上取得无监督方法中的最高准确率。

本小节核心结论是：类条件源域可靠性加权能够显著改善 WJDOT 在多源留一域场景中的稳定性和整体性能。

\section{方法间综合对比与准确率汇总}

\subsection{单源完整基线对比与准确率汇总}

本小节汇总单源跨工况实验的完整基线。参与比较的方法包括 SourceOnly、DSAN、CDAN-TS、CoDATS、DeepJDOT、TPU-\allowbreak{}Deep\allowbreak{}JDOT、CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 和 Target-ref。其中 Target-ref 使用目标域训练部分标签进行监督训练，仅作为有标签参考，不参与无监督方法公平排序。

\begin{table}[htbp]
    \centering
    \caption{单源跨工况完整基线的平均诊断性能}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figs/ch6_single_source_method_mean_accuracy.pdf}
    \caption{单源跨工况完整基线平均目标域准确率柱状图}
    \label{fig:single_source_accuracy_bar}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{figs/ch6_single_source_heatmap.pdf}
    \caption{单源跨工况完整基线逐任务目标域准确率热力图}
    \label{fig:single_source_heatmap}
\end{figure}

表 \ref{tab:single_average_results}、图 \ref{fig:single_source_accuracy_bar} 和图 \ref{fig:single_source_heatmap} 表明，SourceOnly 的平均准确率仅为 55.88\%，说明单一生产模式监督训练难以直接泛化到目标模式。传统领域自适应方法中，CoDATS 的平均准确率为 74.30\%，是较强的时间序列对抗适配基线；DeepJDOT 的平均准确率为 68.88\%，说明普通联合分布最优传输在部分跨工况方向上仍受到错误匹配影响。TPU-\allowbreak{}Deep\allowbreak{}JDOT 和 CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 分别达到 80.54\% 和 82.02\%，其中 CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 在全部 6 个单源迁移方向上取得无监督方法中的最高准确率。

本小节核心结论是：单源主线呈现 DeepJDOT $\rightarrow$ TPU-\allowbreak{}Deep\allowbreak{}JDOT $\rightarrow$ CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 的稳定递进关系，且最终方法整体优于传统单源基线。

\subsection{多源完整基线对比与准确率汇总}

本小节汇总五源留一域设置下的完整基线。参与比较的方法包括 SourceOnly、CoDATS、WJDOT、CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 和 Target-ref。两源结果用于补充检验小规模多源情形，正文综合对比以覆盖 6 个目标生产模式的五源主设置为准。

\begin{table}[htbp]
    \centering
    \caption{五源留一域完整基线的平均诊断性能}
    \label{tab:multi_average_results}
    \footnotesize
    \setlength{\tabcolsep}{4pt}
    \begin{tabular}{l c c c c}
        \hline
        方法 & 准确率 & 宏平均F1 & 平衡准确率 & 无监督第一数 \\
        \hline
        ...
        CA-\allowbreak{}CCSR-\allowbreak{}WJDOT & 85.50$\pm$2.50 & 84.30$\pm$2.58 & 85.51$\pm$2.54 & 6/6 \\
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figs/ch6_multi_source_method_mean_accuracy.pdf}
    \caption{五源留一域完整基线平均目标域准确率柱状图}
    \label{fig:multi_source_accuracy_bar}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{figs/ch6_multi_source_heatmap.pdf}
    \caption{五源留一域完整基线逐目标域准确率热力图}
    \label{fig:multi_source_heatmap}
\end{figure}

表 \ref{tab:multi_average_results}、图 \ref{fig:multi_source_accuracy_bar} 和图 \ref{fig:multi_source_heatmap} 表明，CoDATS 相比 SourceOnly 取得明显提升，说明卷积式时间序列表征和领域对抗约束能够有效利用多源信息。WJDOT 的平均准确率为 78.37\%，低于 CoDATS，说明仅依赖全局源域权重时，逐源联合分布传输仍可能受到源域差异和类别级负迁移影响。CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 达到 85.50$\pm$2.50\%，高于 CoDATS 和 WJDOT，并在 6/6 个目标域上取得无监督方法第一。

本小节核心结论是：CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 在五源留一域设置中整体优于 CoDATS 和 WJDOT。

\section{代表性场景可视化与机制分析}

\subsection{单源代表场景分析}

为避免在正文中堆叠所有任务的可视化结果，本小节选取较困难的单源任务 $m_5\rightarrow m_2$ 作为代表场景，分别展示目标域混淆矩阵和域融合特征可视化结果。该任务用于观察 DeepJDOT、TPU-\allowbreak{}Deep\allowbreak{}JDOT 和 CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 在类别边界和源目标特征融合上的递进变化。
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{置信均衡伪标签方法}
    \end{subfigure}
    \caption{单源任务 $m_5\rightarrow m_2$ 中代表方法的目标域混淆矩阵}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{置信均衡伪标签方法}
    \end{subfigure}
    \caption{单源任务 $m_5\rightarrow m_2$ 中代表方法的域融合特征可视化}
    \label{fig:single_tsne_domain_grid}
\end{figure}

图 \ref{fig:single_confusion_matrix_grid} 和图 \ref{fig:single_tsne_domain_grid} 显示，SourceOnly 的类别错误分布较分散，源域和目标域特征也存在明显分离；DeepJDOT 能够拉近部分源目标样本，但仍存在类别混淆；TPU-\allowbreak{}Deep\allowbreak{}JDOT 通过非平衡传输和原型约束增强对角线响应；CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 进一步利用高置信目标样本，使目标域样本更集中地进入可判别区域。

本小节核心结论是：单源代表场景中的混淆矩阵和特征可视化结果与主结果一致，说明性能提升主要来自类别边界更清晰和源目标特征融合更可靠。

\subsection{多源代表场景分析}

多源代表场景选取五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$。该任务与单源代表场景具有相同目标域 $m_2$，便于观察在目标域困难时，多源方法是否能够通过可靠源域选择改善类别识别。
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_multi_codats_5src_to_2_confusion.pdf}
        \caption{时间序列对抗方法}
    \end{subfigure}
    \vspace{0.4em}
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_multi_wjdot_5src_to_2_confusion.pdf}
        \caption{多源加权传输}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{类条件可靠性加权方法}
    \end{subfigure}
    \caption{五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中代表方法的目标域混淆矩阵}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_multi_codats_5src_to_2_tsne_domain.pdf}
        \caption{时间序列对抗方法}
    \end{subfigure}
    \vspace{0.4em}
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_multi_wjdot_5src_to_2_tsne_domain.pdf}
        \caption{多源加权传输}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{类条件可靠性加权方法}
    \end{subfigure}
    \caption{五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中代表方法的域融合特征可视化}
    \label{fig:multi_tsne_domain_grid}
\end{figure}

图 \ref{fig:multi_confusion_matrix_grid} 和图 \ref{fig:multi_tsne_domain_grid} 表明，SourceOnly 和 WJDOT 在目标域 $m_2$ 上仍存在较多类别混淆，说明简单合并源域或仅学习全局源域权重都不足以完全避免负迁移。CoDATS 通过时间序列表征和领域对抗约束改善整体识别能力；CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 则进一步使目标域样本靠近对其类别更可靠的源域特征区域，与五源平均性能提升相一致。

本小节核心结论是：多源代表场景说明 CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 的优势不是无差别混合所有源域，而是更有选择地利用可靠源域信息。

\subsection{类条件源域权重机制分析}

为使机制解释服务于结果，本小节仅保留一张源域--类别权重热力图，用于说明 CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 为什么能够在五源设置中优于 WJDOT。图 \ref{fig:class_source_alpha_heatmap} 展示了五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中，不同源域对不同故障类别的类条件权重。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{figs/ch6_class_source_alpha_heatmap.pdf}
    \caption{五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中本文多源方法的源域--类别权重热力图}
    \label{fig:class_source_alpha_heatmap}
\end{figure}

图 \ref{fig:class_source_alpha_heatmap} 反映出，不同故障类别并不总是依赖同一个源域：部分类别更偏向 $m_1$ 或 $m_5$，部分类别则更多利用 $m_4$ 或 $m_6$。这说明 CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 的改进不是简单增加一个全局源域权重，而是在类别层面对源域贡献进行重新分配，从而解释了其相对 WJDOT 的平均性能提升和 6/6 目标域第一结果。

本小节核心结论是：源域--类别权重矩阵提供了对多源结果的必要解释，但正文只需保留该代表性机制图，其他诊断曲线和中间状态更适合作为附录候选。

\subsection{主线目标达成分析}

综合 6.3 节至 6.5 节结果，本文三条方法主线分别对应单源可靠对齐、目标域无标签样本利用和多源选择性迁移。单源实验中，TPU-\allowbreak{}Deep\allowbreak{}JDOT 相比 DeepJDOT 在平均准确率、宏平均 F1 和平衡准确率上均取得明显提升，说明时序原型、非平衡传输和时序统计代价能够缓解普通联合分布最优传输中的错误匹配；CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 在此基础上继续获得一致增益，说明冻结教师锚点和多证据置信均衡伪标签机制能够进一步利用目标域无标签样本。单源代表场景中的混淆矩阵和域融合特征可视化也与该趋势一致。

多源实验中，CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 在五源留一域完整基线中取得最高无监督平均准确率，并在 6 个目标域上均达到无监督方法第一。与 WJDOT 相比，该结果说明全局源域权重不足以刻画不同故障类别对源域的差异化需求，而类条件源域可靠性矩阵能够提供更细粒度的选择性迁移能力。源域--类别权重热力图进一步说明，不同故障类别确实依赖不同源域信息，从而为多源方法的性能提升提供了可解释支撑。

总体来看，本文方法基本达成了在目标域无标签条件下提升跨工况故障诊断性能并解释源域可靠性差异的目标。但与 Target-ref 仍存在一定差距，说明完全无标签条件下的目标域类别校准仍然困难；在部分复杂目标域中，伪标签语义偏差和多源负迁移仍可能影响最终性能。这些现象也构成后续研究弱监督目标域校准和更稳健源域筛选策略的依据。

\section{本章小结}

本章围绕田纳西伊斯曼过程领域自适应数据集，在统一实验协议下对单源和多源跨工况故障诊断方法进行了实验验证。单源实验表明，TPU-\allowbreak{}Deep\allowbreak{}JDOT 相比 DeepJDOT 明显提升平均准确率、宏平均 F1 和平衡准确率，验证了时序原型非平衡联合分布对齐的有效性；CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 相比 TPU-\allowbreak{}Deep\allowbreak{}JDOT 进一步取得一致增益，验证了冻结教师锚点和多证据置信均衡伪标签机制的作用。

多源实验表明，CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 在五源留一域设置中达到 85.50$\pm$2.50\% 的平均准确率，并在 6/6 个目标域上取得无监督方法第一，整体优于 CoDATS 和 WJDOT。该结果说明，类条件源域可靠性加权能够缓解 WJDOT 全局源域权重粒度过粗的问题，并提升多源选择性迁移能力。

总体而言，本章正文保留了单源和多源主结果表、平均准确率柱状图、逐任务准确率热力图、代表场景的混淆矩阵与特征可视化结果，以及一张源域--类别权重热力图。其他诊断材料不进入正文，可作为附录或内部分析材料。实验结果共同说明，本文方法能够在目标域无标签条件下提升工业过程跨工况故障诊断性能，并为源域可靠性差异提供必要解释。
...
\cleardoublepage
\chapter{SEED42 主线算法参数表}
\label{app:seed42_mainline_config}

本附录只列出本文三条主线创新方法在 \texttt{SEED42} 主实验中采用的主要算法参数。表中符号与正文第三章至第五章保持一致；目标域标签仅用于最终评价，不参与训练、早停、模型选择或参数确定。

为避免附录冗长，表中只列出影响三条创新主线的主要数值参数。若某一参数在特定场景中有覆盖取值，则以场景覆盖表为准；未列出的基线方法参数不属于本文创新方法参数，故不在本附录展开。
...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.28\textwidth}>{\raggedright\arraybackslash}p{0.38\textwidth}}
    ...
    目标域训练标签 & $y^t$ & 训练和模型选择阶段不可见，仅最终评价使用 \\
    故障类别数 & $N_c$ & $29$ \\
    源域与目标域折号 & $F_s,F_t$ & $1,1$ \\
    评估间隔 & $I_{\mathrm{eval}}$ & $5$ 个训练轮 \\
    模型选择间隔 & $I_{\mathrm{sel}}$ & $5$ 个训练轮 \\
    早停检查间隔 & $I_{\mathrm{es}}$ & $5$ 个训练轮 \\
    单源最小训练轮数 & $E_{\min}^{\mathrm{single}}$ & $120$ \\
    多源最小训练轮数 & $E_{\min}^{\mathrm{multi}}$ & $80$ \\
\end{longtable}
}
...
{\footnotesize
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.19\textwidth}}
    \caption{TPU-DeepJDOT 与 CBTPU-DeepJDOT 主参数}
    \label{tab:seed42_single_method_params}\\
    \toprule
    参数含义 & 符号 & TPU-\allowbreak{}Deep\allowbreak{}JDOT & CBTPU-\allowbreak{}Deep\allowbreak{}JDOT \\
    \midrule
    \endfirsthead
    \toprule
    参数含义 & 符号 & TPU-\allowbreak{}Deep\allowbreak{}JDOT & CBTPU-\allowbreak{}Deep\allowbreak{}JDOT \\
...
{\footnotesize
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.16\textwidth}>{\raggedright\arraybackslash}p{0.27\textwidth}>{\raggedright\arraybackslash}p{0.49\textwidth}}
    \caption{单源场景级参数覆盖}
    ...
    $m_1\rightarrow m_2$ & TPU：$\lambda_{\mathrm{ot}}^0,\nu_{\mathrm{align}},\nu_p,S_p,\nu_{\mathrm{temp}},S_{\mathrm{temp}}$；CBTPU：$\lambda_{\mathrm{ot}}^0,\lambda_{\mathrm{pl}}^0,\vartheta_{\mathrm{start}},\vartheta_{\mathrm{end}},a_{\max},\lambda_{\mathrm{cons}}^0$ & TPU：$0.62,700,700,900,700,900$；CBTPU：$0.50,0.008,0.94,0.88,0.40,0.014$ \\
...
表 \ref{tab:seed42_ca_ccsr_params} 列出 CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 在二源和五源主线中的核心算法参数。二源主线采用统一参数；五源主线的目标域相关覆盖见表 \ref{tab:seed42_ca_scene_overrides}。

{\footnotesize
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.21\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}}
    ...
    对齐损失基础权重 & $\lambda_{\mathrm{ot}}^0$ & $0.48$ & $0.48$ \\
    ...
    覆盖融合系数 & $\lambda_{\mathrm{override}}$ & $0.35$ & $0.35$ \\
    覆盖置信/熵裕度 & $\Delta_{\mathrm{conf}},\Delta_H$ & $0.08,0.04$ & $0.08,0.04$ \\
    融合系数范围 & $\eta_{\min},\eta_{\max}$ & $0,0.45$ & $0,0.45$ \\
    学生保护裕度 & $\Delta_{\mathrm{guard}}^{\mathrm{conf}},\Delta_{\mathrm{guard}}^H$ & -- & $0.05,0.02$ \\
    WJDOT 锚点融合权重 & $\alpha_{\mathrm{anchor}}$ & $0.28$ & $0.41$ \\
\end{longtable}
}
...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.21\textwidth}>{\raggedright\arraybackslash}p{0.71\textwidth}}
    \caption{五源 CA-CCSR-WJDOT 场景级参数覆盖}
    ...
    $m_3$ & $E=20$，$\eta_{\mathrm{lr}}=3.0\times10^{-5}$，$\lambda_{\mathrm{ot}}^0=0.45$，$\lambda_s=0.70$，$S_{\mathrm{align}}=300$，$N_{\mathrm{rel}}=1440$，$(w_p,w_o,w_h,w_e)=(0.30,0.35,0.20,0.15)$，$\lambda_d=0.15$，$\lambda_{\mathrm{WJDOT}}=\lambda_{\mathrm{CCSR}}=0.04$，$\lambda_{\mathrm{tea}}=0.20$，$\lambda_{\mathrm{same}}=0.05$，$\lambda_{\mathrm{override}}=0.60$，$(\eta_{\min},\eta_{\max})=(0,0.70)$。 \\
    $m_4$ & 采用表 \ref{tab:seed42_ca_ccsr_params} 五源通用取值。 \\
    $m_5$ & 继承 $m_3$ 的覆盖参数，但 $\lambda_{\mathrm{same}}=\lambda_{\mathrm{override}}=0$。 \\
    $m_6$ & $E=30$，$\lambda_s=0.82$，$\lambda_d=0.10$，$\lambda_{\mathrm{WJDOT}}=\lambda_{\mathrm{CCSR}}=0.060$，$\lambda_{\mathrm{tea}}=0.08$；融合参数采用 $m_2$ 的覆盖取值。 \\
\end{longtable}
}

以上参数表仅用于说明本文主线实验中的算法超参数取值；未在表中列出的量按正文算法定义取值。
