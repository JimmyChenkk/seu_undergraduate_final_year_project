...
综上，本章方法的总体目标函数为
\begin{equation}
    \mathcal{L}_{\mathrm{TPU\_DPJDOT}}
    =
    \mathcal{L}_{\mathrm{src}}
    +
...
教师模型一致性学习通过比当前学生模型更稳定的预测分布，为无标签样本提供软监督信号。Mean Teacher 方法表明，权重平均教师模型可为无标签样本提供更平滑的一致性目标\cite{tarvainen2017meanteacher}。在本文单源主线中，第四章方法进一步利用第三章已经训练得到的 TPU\_DPJDOT 模型作为教师锚点。设第三章模型参数为 $\Theta_{\mathrm{TPU\_DPJDOT}}$，则教师模型和学生模型初始化为
\begin{equation}
    \bar{\Theta}\leftarrow \Theta_{\mathrm{TPU\_DPJDOT}},
    \quad
    \Theta\leftarrow \Theta_{\mathrm{TPU\_DPJDOT}},
\end{equation}
...
\begin{equation}
    ...
    (1-\xi)\Theta,
\end{equation}
其中 $\xi$ 为 EMA 衰减系数。本文实验采用冻结 TPU\_DPJDOT 教师锚点作为主要设置，EMA 更新仅作为未加载锚点时的通用实现形式。
...
教师预测语义分布 $q_j^{\mathrm{cls}}$ 来自教师模型对目标弱增强样本的预测。在本文实验主线中，该教师模型为第三章 TPU\_DPJDOT 检查点加载得到的冻结教师锚点；若未加载锚点，则可由 EMA 教师提供。相比当前正在接受伪标签微调的学生模型，冻结教师锚点保留了前一阶段较稳定的跨域对齐边界，因此 $q_j^{\mathrm{cls}}$ 既提供类别预测证据，也作为强增强一致性学习的目标分布。
...
本章方法的总损失为
\begin{equation}
    \mathcal{L}_{\mathrm{CBTPU\_DPJDOT}}
    =
    \mathcal{L}_{\mathrm{src}}
    +
    \lambda_{\mathrm{supcon}}(\nu)\mathcal{L}_{\mathrm{supcon}}
...
\begin{algorithm}[htbp]
    ...
    初始化学生模型、教师模型和源域类别原型；若存在 TPU\_DPJDOT 阶段检查点，则加载为冻结教师锚点并初始化学生模型\;
    \For{$\nu=1,2,\cdots,N_{\mathrm{iter}}$}{
...
本章训练阶段的总体目标函数为
\begin{equation}
    \mathcal{L}_{\mathrm{CA\_CCSR\_WJDOT}}
    =
...
本文方法名称按全文统一约定书写。第三章方法记为 TPU\_DPJDOT，第四章方法记为 CBTPU\_DPJDOT，第五章方法记为 CA\_CCSR\_WJDOT。DeepJDOT、WJDOT、CoDATS 仅作为基线或基础方法名称保留。
...
\section{单源时序原型非平衡联合分布对齐实验}

本节验证第三章提出的时序原型非平衡联合分布对齐机制，叙事重点是 DeepJDOT 到 TPU\_DPJDOT 的递进关系。正文结果均采用固定 seed42、固定第一折协议和冻结的主线结果；表中“均值 $\pm$ 标准差”表示不同迁移任务或目标域之间的均值与场景标准差。其他随机性条件下的补充结果不在正文展开。

\subsection{单源实验结果对比}

表 \ref{tab:single_average_results} 给出单源跨工况实验的主结果。参与比较的方法包括 SourceOnly、DSAN、CDAN-TS、CoDATS、DeepJDOT、TPU\_DPJDOT、CBTPU\_DPJDOT 和 Target-ref。其中 Target-ref 使用目标域训练部分标签进行监督训练，仅作为有标签参考，不参与无监督方法公平排序。为避免后文重复列同一组单源结果，本表同时作为第 6.3 节和第 6.4 节的单源性能依据；本小节只分析 SourceOnly、DeepJDOT 和 TPU\_DPJDOT。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c c c}
        ...
        TPU\_DPJDOT & 80.54$\pm$2.20 & 78.82$\pm$2.18 & 80.64$\pm$2.28 & 0/6 \\
        CBTPU\_DPJDOT & 82.02$\pm$1.64 & 80.73$\pm$1.54 & 82.12$\pm$1.71 & 6/6 \\
...
\begin{figure}[htbp]
    ...
    \caption{单源跨工况逐任务目标域准确率热力图}
    \label{fig:single_source_heatmap}
\end{figure}

表 \ref{tab:single_average_results} 与图 \ref{fig:single_source_heatmap} 表明，SourceOnly 的平均准确率仅为 55.88\%，说明单一生产模式监督训练难以直接泛化到目标模式。DeepJDOT 的平均准确率为 68.88\%，相比 SourceOnly 有明显提升，但普通平衡联合分布传输仍容易在类别组成不一致的小批量中产生错误匹配。TPU\_DPJDOT 的平均准确率达到 80.54\%，相较 DeepJDOT 提升 11.66 个百分点，宏平均 F1 和平衡准确率分别提升 12.78 和 11.56 个百分点，并在 6 个单源迁移任务中均高于 DeepJDOT。

上述结果说明，非平衡传输、源域类别原型和时序统计代价并非只带来局部场景收益，而是在全部单源迁移方向上稳定增强了 DeepJDOT 的类别结构保持能力。

单源代表场景的混淆矩阵和域融合特征在第 6.4 节与 CBTPU\_DPJDOT 一并呈现，以保持同一代表场景下的图像比较完整。

\subsection{时序原型非平衡联合分布对齐机制分析}

DeepJDOT 使用平衡最优传输时，每个小批量内的源域和目标域样本质量需要被完整匹配。对于 TEP 跨工况任务，这一约束会在目标域类别组成不均衡或预测尚不稳定时放大错误传输。TPU\_DPJDOT 将对齐损失改为非平衡联合分布传输，使不可靠传输质量可以被削减；同时将源域类别原型距离、原始时间窗口统计代价和源域监督对比约束引入联合代价，使传输计划同时受到特征几何、故障类别中心和时序动态信息约束。

以 $m_5\rightarrow m_2$ 为例，训练诊断中 DeepJDOT 的传输质量始终固定为完整匹配，而 TPU\_DPJDOT 的非平衡传输质量不再被强制固定为 1，后期保持在约 0.69 左右；同时，目标域训练样本的平均预测熵低于 DeepJDOT，对应目标域预测置信度更高。该现象说明 TPU\_DPJDOT 的收益不是简单增大传输损失权重，而是通过非平衡传输和原型代价共同抑制了低可靠匹配。

\section{单源多证据置信均衡伪标签实验}

本节验证第四章提出的多证据置信均衡伪标签机制，叙事重点是 TPU\_DPJDOT 到 CBTPU\_DPJDOT 的递进关系。为使单源图像安排与方法递进一致，本节在同一代表场景中同时展示 SourceOnly、DeepJDOT、TPU\_DPJDOT 和 CBTPU\_DPJDOT。

\subsection{单源实验结果对比}

由表 \ref{tab:single_average_results} 可知，CBTPU\_DPJDOT 的平均准确率、宏平均 F1 和平衡准确率分别为 82.02\%、80.73\% 和 82.12\%，相较 TPU\_DPJDOT 继续提升 1.48、1.91 和 1.48 个百分点。逐任务结果中，CBTPU\_DPJDOT 在 6 个单源迁移方向上均高于 TPU\_DPJDOT，并在全部 6 个单源任务中取得无监督方法第一。

这一结果说明，多证据置信均衡伪标签学习并不是替代前一阶段对齐机制，而是在 TPU\_DPJDOT 已建立较可靠源目标对应关系后，进一步利用目标域无标签样本微调目标域决策边界。

\subsection{代表场景混淆矩阵分析}

代表场景选取较困难的单源任务 $m_5\rightarrow m_2$。该目标域与模式5存在较明显分布差异，适合观察单源方法是否能够减少类别间误判。图 \ref{fig:single_source_confusion_matrix_grid} 展示 SourceOnly、DeepJDOT、TPU\_DPJDOT 和 CBTPU\_DPJDOT 的目标域混淆矩阵。
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{CBTPU\_DPJDOT}
    \end{subfigure}
    \caption{单源任务 $m_5\rightarrow m_2$ 中单源主线方法的目标域混淆矩阵}
    \label{fig:single_source_confusion_matrix_grid}
\end{figure}

图 \ref{fig:single_source_confusion_matrix_grid} 显示，SourceOnly 在目标域上存在较分散的非对角误判，说明仅依赖源域监督训练难以处理生产模式偏移；DeepJDOT 改善了部分故障类识别，但仍保留较明显的类别混淆。TPU\_DPJDOT 的对角线响应更集中，说明非平衡联合传输和原型约束能够减少不可靠样本强制匹配对类别边界的干扰。CBTPU\_DPJDOT 相比 TPU\_DPJDOT 的代表场景准确率由 83.60\% 提升到 83.77\%，图中整体结构变化不剧烈，主要体现为局部非对角误判继续减弱；因此第四章方法的主要证据来自 6 个单源任务上的一致正向增益。

\subsection{域融合特征可视化分析}

图 \ref{fig:single_source_tsne_domain_grid} 展示同一代表场景的源域和目标域特征分布。该图用于观察方法是否仅提高分类器输出，还是同时改善源目标特征融合状态。
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{CBTPU\_DPJDOT}
    \end{subfigure}
    \caption{单源任务 $m_5\rightarrow m_2$ 中单源主线方法的域融合特征可视化}
    \label{fig:single_source_tsne_domain_grid}
\end{figure}

从图 \ref{fig:single_source_tsne_domain_grid} 可以看出，SourceOnly 的源域与目标域特征分布仍存在明显分离；DeepJDOT 缩小了部分域间距离，但目标域样本仍未充分进入稳定的类别结构中。TPU\_DPJDOT 使更多目标域样本靠近源域类别区域；CBTPU\_DPJDOT 在保持该对齐结构的基础上，使主簇附近的目标样本分布更加紧凑。该趋势与 CBTPU\_DPJDOT 在 6/6 单源任务中取得无监督第一的结果相互印证。

\subsection{多证据置信均衡伪标签机制分析}

CBTPU\_DPJDOT 的关键不是扩大伪标签样本覆盖率，而是控制目标样本进入监督信号的可靠性。该方法同时融合非平衡传输语义分布、冻结教师预测分布和源域原型语义分布，并通过置信度、熵和多证据一致性进行筛选。以 $m_5\rightarrow m_2$ 为例，代表性训练诊断中目标样本接受比例约为 0.55，传输语义与分类器--原型语义的一致率后期约为 0.74，目标域预测熵低于 TPU\_DPJDOT，平均预测置信度更高。

因此，CBTPU\_DPJDOT 的增益可解释为“保守地使用目标域样本”：只有当传输计划、教师锚点和原型距离给出相对一致的语义判断时，目标样本才参与伪标签学习；类别均衡约束则避免高置信类别过度主导训练。该机制解释了为什么 CBTPU\_DPJDOT 的平均提升幅度不如 TPU\_DPJDOT 相对 DeepJDOT 那样大，但能够在所有单源场景上保持正向增益。

\section{多源类条件源域可靠性实验}

本节验证第五章提出的类条件源域可靠性加权机制，叙事重点是 CoDATS / WJDOT 到 CA\_CCSR\_WJDOT 的递进关系。多源实验分为两源和五源两组：两源任务用于检验小规模多源场景，五源留一域任务用于检验源域数量增加时的选择性迁移能力。

\subsection{多源实验结果对比}

表 \ref{tab:two_source_average_results} 给出两源任务的平均诊断性能。CA\_CCSR\_WJDOT 在 3 个两源目标域上均取得无监督方法第一，平均准确率达到 81.21\%，分别高于 CoDATS 和 WJDOT 3.99 和 8.80 个百分点。

\begin{table}[htbp]
    \centering
    \caption{两源跨工况主结果的平均诊断性能}
    ...
    \begin{tabular}{l c c c c}
        ...
        CA\_CCSR\_WJDOT & 81.21$\pm$4.12 & 79.97$\pm$4.50 & 81.32$\pm$4.20 & 3/3 \\
        Target-ref & 91.94$\pm$1.44 & 91.49$\pm$0.92 & 91.98$\pm$1.42 & -- \\
        \hline
    \end{tabular}
\end{table}

表 \ref{tab:multi_average_results} 给出五源留一域任务的平均诊断性能。CA\_CCSR\_WJDOT 在 6 个目标生产模式上均取得无监督方法第一，平均准确率达到 85.50\%，高于 CoDATS 的 83.44\% 和 WJDOT 的 78.37\%。宏平均 F1 和平衡准确率也呈现相同趋势，说明该方法的收益并非只来自部分类别或样本数量较多的类别。

\begin{table}[htbp]
    \centering
    \caption{五源留一域主结果的平均诊断性能}
    ...
    \begin{tabular}{l c c c c}
        ...
        CA\_CCSR\_WJDOT & 85.50$\pm$2.50 & 84.30$\pm$2.58 & 85.51$\pm$2.54 & 6/6 \\
...
\begin{figure}[htbp]
    ...
    \caption{五源留一域逐目标生产模式准确率热力图}
    \label{fig:multi_source_heatmap}
\end{figure}

两组多源结果共同表明，CA\_CCSR\_WJDOT 相比 CoDATS 和 WJDOT 均取得更高目标域性能。图 \ref{fig:multi_source_heatmap} 给出五源留一域任务的逐目标模式结果。CA\_CCSR\_WJDOT 在 6 个目标生产模式上均取得无监督方法最高准确率。CoDATS 提供较强的时间序列表征和领域对抗基线，WJDOT 提供逐源联合分布传输思想。CA\_CCSR\_WJDOT 在此基础上引入类条件源域可靠性，使模型能够针对不同故障类别选择不同源域信息。

\subsection{代表场景混淆矩阵分析}

多源代表场景选取五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$。该任务与单源代表场景具有相同目标域 $m_2$，便于观察在困难目标域下，多源选择性迁移是否能够减少类别混淆。图 \ref{fig:multi_confusion_matrix_grid} 同时展示 SourceOnly、CoDATS、WJDOT 和 CA\_CCSR\_WJDOT，使可视化比较与表中主要多源基线保持一致。
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{CA\_CCSR\_WJDOT}
    \end{subfigure}
    \caption{五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中多源代表方法的目标域混淆矩阵}
    \label{fig:multi_confusion_matrix_grid}
\end{figure}

图 \ref{fig:multi_confusion_matrix_grid} 表明，SourceOnly 虽然已经利用多个源域训练，但在目标域 $m_2$ 上仍存在若干集中误判；CoDATS 的对角线明显增强，说明时间序列表征和领域对抗约束提供了较强多源基线。WJDOT 仍有部分故障类被错误吸收到少数预测类别，反映全局源域权重难以充分刻画类别级源域差异。CA\_CCSR\_WJDOT 在保留 CoDATS 对角线优势的基础上，进一步减弱若干非对角误判，与五源主结果中 6/6 目标域第一的趋势一致。

\subsection{域融合特征可视化分析}

图 \ref{fig:multi_tsne_domain_grid} 展示同一五源代表场景下的域融合特征。四个子图依次对应 SourceOnly、CoDATS、WJDOT 和 CA\_CCSR\_WJDOT。该图用于观察类条件源域可靠性是否改善目标域样本在多源特征空间中的位置。
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{CA\_CCSR\_WJDOT}
    \end{subfigure}
    \caption{五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中多源代表方法的域融合特征可视化}
    \label{fig:multi_tsne_domain_grid}
\end{figure}

从图 \ref{fig:multi_tsne_domain_grid} 可以看出，SourceOnly 下目标域样本在部分区域与源域簇仍有明显距离；CoDATS 已经使多个目标簇靠近源域结构，但局部仍存在目标样本游离。WJDOT 的部分目标样本仍分布在源域簇外或仅靠近少数源域。CA\_CCSR\_WJDOT 使目标域样本更稳定地靠近可靠源域形成的判别区域，说明多源性能提升并非单纯来自源域数量增加，而是来自对源域贡献的选择性利用。

\subsection{类条件源域可靠性机制分析}

为使机制解释服务于结果，正文只保留一张源域--类别权重热力图。图 \ref{fig:class_source_alpha_heatmap} 展示了五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中，不同源域对不同故障类别的类条件权重。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{figs/ch6_class_source_alpha_heatmap.pdf}
    \caption{五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中 CA\_CCSR\_WJDOT 的源域--类别权重热力图}
    \label{fig:class_source_alpha_heatmap}
\end{figure}

图 \ref{fig:class_source_alpha_heatmap} 反映出，不同故障类别并不依赖同一个源域。例如，在该代表场景中，类别1、类别2和类别8对 $m_1$ 的权重较高，类别11和类别27明显偏向 $m_6$，类别24和类别25则更多依赖 $m_4$；同时也有若干类别呈现多个源域共同提供信息的状态。配套诊断中，部分类别的权重熵较低，说明其可靠源域更集中；另一些类别的有效源域数更高，说明这些类别需要更分散的源域支持。相对 WJDOT，CA\_CCSR\_WJDOT 在类别1和类别8等故障类上取得明显召回增益，虽然个别类别仍存在局部下降，但整体目标域准确率、宏平均 F1 和平衡准确率均获得提升。

因此，CA\_CCSR\_WJDOT 的关键不是简单平均源域，也不是只学习一个全局源域权重，而是在“源域--故障类别”层面重新分配迁移贡献。这一机制与两源 3/3 第一、五源 6/6 第一的主结果相对应。

\section{主线目标达成与综合分析}

本节汇总第 6.3 节至第 6.5 节的实验结论，重点说明三条实验主线是否达到预期目标。该节不讨论方法局限，而是从性能递进、跨场景稳定性和统计显著性角度进行综合分析。

\subsection{单源递进主线分析}

单源主线的目标是验证 DeepJDOT $\rightarrow$ TPU\_DPJDOT $\rightarrow$ CBTPU\_DPJDOT 的逐级改进。实验结果表明，DeepJDOT 相比 SourceOnly 能够引入联合分布对齐，但在部分场景中仍受到平衡传输和类别混淆影响；TPU\_DPJDOT 将平均准确率从 68.88\% 提升到 80.54\%，并在 6/6 单源任务上优于 DeepJDOT，说明时序原型非平衡联合分布对齐是单源主线中的主要增益来源；CBTPU\_DPJDOT 进一步达到 82.02\%，并在 6/6 单源任务中取得无监督方法第一，说明多证据置信均衡伪标签能够在可靠对齐基础上继续改善目标域决策边界。

从代表场景看，混淆矩阵和域融合特征可视化也呈现同样递进关系：SourceOnly 的目标域误判分散，DeepJDOT 改善部分对齐，TPU\_DPJDOT 增强类别结构，CBTPU\_DPJDOT 进一步稳定目标域样本利用。

\subsection{多源选择性迁移分析}

多源主线的目标是验证 CoDATS / WJDOT $\rightarrow$ CA\_CCSR\_WJDOT 的选择性迁移收益。两源任务中，CA\_CCSR\_WJDOT 达到 81.21\% 平均准确率，并在 3/3 目标域上取得无监督第一；五源留一域任务中，CA\_CCSR\_WJDOT 达到 85.50\% 平均准确率，并在 6/6 目标域上取得无监督第一。该结果说明，随着源域数量增加，简单利用多源数据并不足够，模型还需要判断不同源域对不同故障类别是否可靠。

源域--类别权重热力图进一步说明，CA\_CCSR\_WJDOT 的提升与机制设计存在对应关系：不同故障类别具有不同的源域依赖模式，类条件源域可靠性矩阵能够将这些差异显式纳入训练和融合过程，从而缓解全局源域权重粒度过粗带来的负迁移风险。

\subsection{统计显著性验证}

为验证主线提升是否具有跨迁移场景的一致性，本文采用 Wilcoxon signed-rank test 对相邻主线方法进行配对检验。这里的配对单位是迁移场景，而不是多随机种子重复实验，因此检验结论表述为“跨迁移场景配对显著性”。检验结果如表 \ref{tab:significance_results} 所示。

\begin{table}[htbp]
    \centering
    \caption{主线方法跨迁移场景配对显著性检验结果}
    ...
    \begin{tabular}{l c c c}
        ...
        \hline
        TPU\_DPJDOT vs. DeepJDOT & 单源6任务 & 6/6 & 0.015625 \\
        CBTPU\_DPJDOT vs. TPU\_DPJDOT & 单源6任务 & 6/6 & 0.015625 \\
        CA\_CCSR\_WJDOT vs. WJDOT & 两源与五源共9任务 & 9/9 & 0.001953 \\
        \hline
    \end{tabular}
\end{table}

表 \ref{tab:significance_results} 表明，三条相邻主线比较均呈现跨场景一致正向差异。该结果支持本文实验主张：TPU\_DPJDOT 相比 DeepJDOT 更稳定，CBTPU\_DPJDOT 相比 TPU\_DPJDOT 继续带来一致增益，CA\_CCSR\_WJDOT 相比 WJDOT 在多源任务中具有更强的选择性迁移能力。

\section{本章小结}

本章围绕田纳西伊斯曼过程领域自适应数据集，在统一实验协议下对单源和多源跨工况故障诊断方法进行了实验验证。单源实验表明，TPU\_DPJDOT 相比 DeepJDOT 明显提升平均准确率、宏平均 F1 和平衡准确率，验证了时序原型非平衡联合分布对齐的有效性；CBTPU\_DPJDOT 相比 TPU\_DPJDOT 进一步取得一致增益，并在 6/6 个单源任务中取得无监督方法第一，验证了多证据置信均衡伪标签机制的作用。

多源实验表明，CA\_CCSR\_WJDOT 在两源设置中 3/3 第一，在五源留一域设置中 6/6 第一，并在五源任务上达到 85.50$\pm$2.50\% 的平均准确率，整体优于 CoDATS 和 WJDOT。该结果说明，类条件源域可靠性加权能够缓解 WJDOT 全局源域权重粒度过粗的问题，并提升多源选择性迁移能力。

总体而言，本章按照方法主线递进组织实验叙事，正文保留单源主结果表、单源逐任务热力图、两源和五源多源主结果表、五源逐目标热力图、代表场景混淆矩阵与特征可视化、源域--类别权重热力图以及显著性检验表。实验结果共同说明，本文方法能够在目标域无标签条件下提升工业过程跨工况故障诊断性能，并为源域可靠性差异提供必要解释。
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.42\textwidth}>{\raggedright\arraybackslash}p{0.22\textwidth}@{}}
    ...
    $\Theta,\bar{\Theta},\Theta_{\mathrm{TPU\_DPJDOT}}$ & 学生模型、教师模型和 TPU\_DPJDOT 阶段模型参数集合 & $\Theta=(\theta,\phi)$，$\bar{\Theta}=(\bar{\theta},\bar{\phi})$ \\
    ...
    $\mathcal{L}_{\mathrm{TPU\_DPJDOT}}$、$\mathcal{L}_{\mathrm{CBTPU\_DPJDOT}}$、$\mathcal{L}_{\mathrm{CA\_CCSR\_WJDOT}}$ & 第三章、第四章和第五章方法的总体损失 & 分别对应三条方法线 \\
...
本附录只列出本文三条主线创新方法在主实验中采用的主要算法参数。表中符号与正文第三章至第五章保持一致；目标域标签仅用于最终评价，不参与训练、早停、模型选择或参数确定。
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.19\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}@{}}
    \caption{TPU\_DPJDOT 与 CBTPU\_DPJDOT 主参数}
    \label{tab:seed42_single_method_params}\\
    \toprule
    参数含义 & 符号 & TPU\_DPJDOT & CBTPU\_DPJDOT \\
    \midrule
    \endfirsthead
    \toprule
    参数含义 & 符号 & TPU\_DPJDOT & CBTPU\_DPJDOT \\
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.14\textwidth}>{\raggedright\arraybackslash}p{0.35\textwidth}>{\raggedright\arraybackslash}p{0.43\textwidth}@{}}
    ...
    $m_1\rightarrow m_2$ & TPU\_DPJDOT：$\lambda_{\mathrm{ot}}^0,\nu_{\mathrm{align}},\nu_p,S_p,\nu_{\mathrm{temp}},S_{\mathrm{temp}}$；CBTPU\_DPJDOT：$\lambda_{\mathrm{ot}}^0,\lambda_{\mathrm{pl}}^0,\vartheta_{\mathrm{start}},\vartheta_{\mathrm{end}},a_{\max},\lambda_{\mathrm{cons}}^0$ & TPU\_DPJDOT：$0.62,700,700,900,700,900$；CBTPU\_DPJDOT：$0.50,0.008,0.94,0.88,0.40,0.014$ \\
...
表 \ref{tab:seed42_ca_ccsr_params} 列出 CA\_CCSR\_WJDOT 在二源和五源主线中的核心算法参数。二源主线采用统一参数；五源主线的目标域相关覆盖见表 \ref{tab:seed42_ca_scene_overrides}。
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.13\textwidth}>{\raggedright\arraybackslash}p{0.81\textwidth}@{}}
    \caption{五源 CA\_CCSR\_WJDOT 场景级参数覆盖}
