...
第 6.3 节从代表场景出发，分别验证三类方法自身的有效性；本节回到全部迁移任务，使用两张热力图和平均性能表说明本文创新方法相对其他基线方法的整体优势。正文结果均采用固定随机种子、固定第一折划分和统一实验协议；表中“均值 $\pm$ 标准差”表示不同迁移任务或目标域之间的均值与场景标准差。Target-ref 使用目标域训练部分标签进行监督训练，仅作为有标签参考，不参与无监督方法公平排序。表中“无监督第一数”统计对应方法在迁移任务中取得无监督方法最高准确率的次数，Target-ref 不纳入该计数。
...
图 \ref{fig:multi_source_heatmap} 给出两源任务和五源留一域任务的逐任务准确率热力图。该图同时覆盖 3 个两源目标域和 6 个五源目标域，用于观察源域数量变化后不同方法是否能够稳定适应目标生产模式。SourceOnly 说明直接合并多源监督数据仍然不足，CoDATS 作为时间序列领域对抗基线具有较强表现，WJDOT 引入逐源联合分布传输但全局源域权重粒度仍然偏粗。CA-CCSR-WJDOT 在两源和五源任务中均取得无监督方法最高准确率，说明类条件源域可靠性加权能够在多源场景下提供更稳定的选择性迁移。

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1\textwidth]{figs/ch6_multi_source_heatmap.pdf}
    \caption{两源与五源多源跨工况任务逐任务目标域准确率热力图}
...
总体而言，本章按照方法递进关系组织实验叙事，正文保留单源主结果表、单源逐任务热力图、两源和五源多源主结果表、多源逐任务汇总热力图、代表场景混淆矩阵与特征可视化、源域--类别权重热力图以及显著性检验表。实验结果共同说明，本文方法能够在目标域无标签条件下提升工业过程跨工况故障诊断性能，并为源域可靠性差异提供必要解释。
