...
工业过程故障诊断方法大体可分为基于机理模型的方法、基于信号处理的方法和基于数据驱动的方法。基于机理模型的方法依赖系统数学模型或先验知识，通过残差分析、状态估计和控制理论判断系统是否异常。这类方法可解释性较强，但在复杂工业过程中，精确建立机理模型往往十分困难，尤其是当系统具有强非线性、多变量耦合和复杂控制结构时，模型构建与参数辨识成本较高。

基于信号处理的方法通常通过时域、频域或时频域分析提取故障特征，再结合分类器进行故障识别。该类方法在旋转机械和传感器信号分析研究中较为常见，但其性能依赖人工特征设计，对不同设备和不同工况的适应能力有限。随着工业数据规模增大和计算能力提升，基于机器学习和深度学习的数据驱动方法逐渐成为故障诊断研究的主流方向。支持向量机、随机森林、人工神经网络、卷积神经网络和循环神经网络等方法能够从历史数据中学习故障模式，在多类故障诊断任务中表现出较高分类性能\cite{lei2020applications}。

在工业过程故障诊断中，过程数据通常表现为多变量时间序列。不同变量之间存在复杂相关关系，同一故障在不同时间阶段可能呈现不同动态响应。因此，如何从多变量时序数据中提取稳定、可判别的故障特征，是数据驱动诊断方法的关键。全卷积网络、长短期记忆网络、注意力机制和 Transformer 等结构已被用于时间序列故障诊断，以增强模型对局部趋势、长期依赖和变量间关系的建模能力。
...
迁移学习在工业故障诊断中的应用主要有两类思路。第一类是预训练与微调，即先利用源域有标签数据训练特征提取器，再使用少量目标域有标签样本进行微调。这类方法实现较为直接，但需要目标域标签，在目标域完全无标签的场景下难以使用。第二类是特征对齐，即通过统计距离、对抗学习或最优传输等方法缩小源域与目标域特征分布差异，使源域训练得到的分类器能够迁移到目标域。相比监督微调，特征对齐更适合目标域标签缺失的场景。
...
针对目标域伪标签不可靠问题，本章提出多证据置信均衡伪标签方法。该方法在第三章模型基础上加入教师--学生锚点结构、强弱增强一致性、多证据语义融合和类别均衡伪标签学习。在本文实验主线中，教师模型由同一迁移场景下训练得到的 TPU-\allowbreak{}Deep\allowbreak{}JDOT 检查点初始化，并作为冻结教师锚点；学生模型也从该检查点初始化，再通过多证据伪标签目标进行保守微调。训练过程不依赖目标域标签，而是融合三类证据：由非平衡传输计划得到的传输语义分布、由教师锚点得到的预测语义分布，以及由源域类别原型得到的原型语义分布。只有当多种证据一致、置信度较高且传输语义熵较低时，目标样本才参与伪标签学习。方法整体结构如图 \ref{fig:ch4_cbtpu_pseudolabel} 所示。
...
\subsection{教师锚点与一致性学习原理}

教师模型一致性学习通过比当前学生模型更稳定的预测分布，为无标签样本提供软监督信号。Mean Teacher 方法表明，权重平均教师模型可为无标签样本提供更平滑的一致性目标\cite{tarvainen2017meanteacher}。在本文单源主线中，第四章方法进一步利用第三章已经训练得到的 TPU-\allowbreak{}Deep\allowbreak{}JDOT 模型作为教师锚点。设第三章模型参数为 $\Theta_{\mathrm{TPU}}$，则教师模型和学生模型初始化为
\begin{equation}
    \bar{\Theta}\leftarrow \Theta_{\mathrm{TPU}},
    \quad
    \Theta\leftarrow \Theta_{\mathrm{TPU}},
\end{equation}
其中 $\Theta=(\theta,\phi)$ 为学生模型参数集合，$\bar{\Theta}=(\bar{\theta},\bar{\phi})$ 为教师模型参数集合。教师锚点不使用目标域标签训练，其来源为前一阶段无监督领域自适应模型；在启用冻结教师锚点时，$\bar{\Theta}$ 在后续训练中保持不变，用于防止伪标签微调破坏第三章已获得的稳定决策边界。

若未加载外部教师锚点，也可以退化为普通 EMA 教师形式：
\begin{equation}
    \bar{\Theta}
    \leftarrow
    \xi\bar{\Theta}
    ...
    (1-\xi)\Theta,
\end{equation}
其中 $\xi$ 为 EMA 衰减系数。本文实验采用冻结 TPU 教师锚点作为主要设置，EMA 更新仅作为未加载锚点时的通用实现形式。
...
\begin{equation}
    ...
    \right),
\end{equation}
其中 $T_{\mathrm{tea}}$ 为教师温度系数。该分布作为目标域预测语义证据，同时也用于强弱增强一致性约束。由于教师锚点来自第三章模型，第四章的伪标签学习是在已有单源对齐结果之上的二阶段细化，而不是从随机初始化重新进行目标域伪标签训练。
...
教师预测语义分布 $q_j^{\mathrm{cls}}$ 来自教师模型对目标弱增强样本的预测。在本文实验主线中，该教师模型为第三章 TPU-\allowbreak{}Deep\allowbreak{}JDOT 检查点加载得到的冻结教师锚点；若未加载锚点，则可由 EMA 教师提供。相比当前正在接受伪标签微调的学生模型，冻结教师锚点保留了前一阶段较稳定的跨域对齐边界，因此 $q_j^{\mathrm{cls}}$ 既提供类别预测证据，也作为强增强一致性学习的目标分布。

本文只使用教师模型对目标无标签样本的输入输出关系，不引入目标标签。教师锚点来自目标标签不可见的第三章训练流程，不直接接受目标标签监督，因此可为目标样本提供较稳定的预测语义证据。
...
第四章方法在训练阶段通过伪标签损失和一致性损失微调学生模型，但最终预测仍需要避免学生模型因局部伪标签噪声而偏离第三章教师锚点。为此，本文在推理阶段采用不使用目标标签的教师--学生预测融合策略。设学生预测概率为 $p_j^{\mathrm{stu}}$，教师锚点预测概率为 $p_j^{\mathrm{tea}}$，二者的最大置信度分别为 $s_j^{\mathrm{stu}}$ 和 $s_j^{\mathrm{tea}}$。
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
其中 $\Delta_{\mathrm{conf}}$ 为固定置信裕度。该规则只有在学生模型明显更有把握时才采用学生预测，否则回退到冻结教师锚点。
...
\end{equation}
时采用学生预测，否则采用教师预测。上述阈值和权重均在实验配置中预先固定，不根据目标域测试准确率进行后验选择，因此仍符合目标域标签不可见的无监督领域自适应设定。
...
\begin{algorithm}[htbp]
    ...
    初始化学生模型、教师模型和源域类别原型；若存在 TPU 阶段检查点，则加载为冻结教师锚点并初始化学生模型\;
    \For{$\nu=1,2,\cdots,N_{\mathrm{iter}}$}{
        ...
        组合各项损失更新学生模型参数；若教师未冻结，则用学生参数滑动更新教师模型参数\;
    }
\end{algorithm}
...
本章在第三章时序原型非平衡联合分布对齐方法基础上，提出了多证据置信均衡伪标签策略。该策略从非平衡传输计划、教师锚点预测和源域类别原型三类来源构建目标域语义分布，并通过一致性判断、JS 差异、动态置信阈值和传输熵筛选可靠目标样本。针对伪标签类别偏置问题，在被接受伪标签集合上进行对数先验调整，缓解少数高置信类别对训练的主导；针对目标时序样本扰动稳定性问题，引入弱增强教师预测和强增强学生预测之间的一致性约束。

本章方法保持无监督领域自适应设定，训练和教师--学生推理融合过程中均不使用目标域标签。目标域标签仅在后续实验章节中用于最终性能评价。通过上述教师锚点、多证据筛选、类别均衡和预测融合机制，模型可以更可靠地利用目标域无标签样本，为单源跨工况故障诊断提供较稳定的目标域决策边界。
...
本章以 Tennessee Eastman Process Domain Adaptation 数据集为实验对象，围绕单源域到单目标域和多源域到单目标域两类跨工况诊断场景展开实验设计与结果分析。具体内容包括数据集来源、生产模式、样本组织形式、变量选择、数据预处理方式、固定第一折评估协议、目标域无标签训练协议和评价指标。在实验设置上，构造由若干生产模式组成的单源双向迁移任务，以及两源域和五源域条件下的多源迁移任务，分别验证第三章、第四章和第五章方法。随后，在相同实验协议下汇总不同方法的诊断性能，并结合准确率、宏平均 F1 值、平衡准确率、混淆矩阵和跨域特征可视化结果分析模型性能和迁移行为。
...
\begin{figure}[htbp]
    ...
    \caption{Tennessee Eastman过程流程示意图}
    \label{fig:tep_process}
\end{figure}

从领域自适应角度看，TEP 数据集的主要特点是包含多个生产模式。不同生产模式对应不同的产品质量比和生产率，会改变过程变量之间的统计关系和数据概率分布。因此，在某一生产模式下训练得到的模型，直接迁移到另一生产模式时可能出现明显性能退化。配套 benchmark 论文通过变量相关矩阵和模式间 Wasserstein 距离分析表明，不同生产模式会造成可观的分布偏移\cite{montesuma2024benchmark}。因此，该数据集适合用于构造单源和多源跨工况领域自适应故障诊断任务。

数据集共包含 6 个生产模式，每个生产模式可视为一个领域。本文沿用基准文献的命名方式，将第 $r$ 个生产模式称为 mode $r$，并在公式和实验场景表中简记为 $m_r$，其中 $r=1,2,\cdots,6$。各生产模式的产品质量比、生产率和样本数如表 \ref{tab:tep_modes} 所示。
...
在上述编号体系下，配套 benchmark 研究进一步指出，mode 2 与其他模式的分布差异相对较大，而 mode 3 与 mode 6、mode 1 与 mode 4、mode 5 之间具有相对更接近的统计特性\cite{montesuma2024benchmark}。
...
单源跨工况实验用于验证第三章和第四章提出的单源领域自适应方法。本文选取 mode 1、mode 2 和 mode 5 构成单源实验子集，并按照双向迁移原则构造任务，即任意两个生产模式之间均进行双向迁移，每个模式既作为源域，也作为目标域。选择 mode 1、mode 2 和 mode 5 的原因是：mode 1 与 mode 5 在 benchmark 分析中属于相对接近的模式组，可用于分析分布偏移较弱任务中的方法增益；mode 2 与其他模式差异较大，可用于检验方法在困难跨工况任务中的适应能力\cite{montesuma2024benchmark}。
...
本节围绕第三章、第四章和第五章提出的三类改良方法展开有效性验证。为突出各创新模块本身的作用，本节仅将所提方法与源域直接迁移方法（SourceOnly）进行对比：若不进行领域自适应，仅依靠源域有标签样本训练得到的模型在目标域上性能较低，而引入本文方法后目标域准确率、宏平均 F1 和平衡准确率明显提高，则说明对应改良机制能够缓解跨工况分布偏移。各结果均采用固定第一折划分，数值为目标域测试集百分比形式。单源图表统一采用同一确定性确认轮结果；单源平均表中的“均值 $\pm$ 标准差”表示 6 个迁移任务的均值和场景标准差，逐场景表直接给出该确认轮的目标域准确率。多源结果仍按三轮重复实验汇总。
% 单源图表来源：runs/single_source_8methods_cbtpu_anchor_deterministic_r7_20260510_seed42

\subsection{时序原型非平衡联合分布对齐有效性}

第三章提出的 TPU-\allowbreak{}Deep\allowbreak{}JDOT 面向单源跨工况诊断任务，在 DeepJDOT 的联合分布最优传输框架基础上引入非平衡传输、源域类别原型、时序统计代价和源域监督对比约束。该方法的直接验证目标是：相较于只在源域监督训练的 SourceOnly，模型是否能够在目标域无标签条件下获得更可靠的诊断边界。

\begin{table}[htbp]
    \centering
    \caption{单源所提方法相对SourceOnly的平均诊断性能}
    \label{tab:ch6_single_sourceonly_proposed_average}
    \small
    \begin{tabular}{l c c c}
        \hline
        方法 & 准确率 & 宏平均F1 & 平衡准确率 \\
        \hline
        SourceOnly & 55.88$\pm$7.67 & 49.97$\pm$7.96 & 55.83$\pm$7.79 \\
        TPU-\allowbreak{}Deep\allowbreak{}JDOT & 80.54$\pm$2.20 & 78.82$\pm$2.18 & 80.64$\pm$2.28 \\
        CBTPU-\allowbreak{}Deep\allowbreak{}JDOT & 82.02$\pm$1.64 & 80.73$\pm$1.54 & 82.12$\pm$1.71 \\
        \hline
    \end{tabular}
\end{table}

由表 \ref{tab:ch6_single_sourceonly_proposed_average} 可知，SourceOnly 的平均准确率仅为 55.88\%，说明 mode 1、mode 2 和 mode 5 之间存在明显跨工况分布偏移。TPU-\allowbreak{}Deep\allowbreak{}JDOT 将平均准确率提高至 80.54\%，较 SourceOnly 提升 24.66 个百分点；宏平均 F1 从 49.97\% 提升至 78.82\%，平衡准确率从 55.83\% 提升至 80.64\%。这表明，第三章方法不仅提高了整体分类正确率，也改善了多类别故障诊断中的均衡识别能力。

\begin{table}[htbp]
    \centering
    \caption{单源所提方法相对SourceOnly的逐场景目标域准确率}
    \label{tab:ch6_single_sourceonly_proposed_scene}
    \footnotesize
    \setlength{\tabcolsep}{4pt}
    \begin{tabular}{c c c c}
        \hline
        迁移任务 & SourceOnly & TPU-DJDOT & CBTPU-DJDOT \\
        \hline
        $m_1\rightarrow m_2$ & 55.03 & 80.07 & 83.07 \\
        $m_1\rightarrow m_5$ & 66.84 & 79.51 & 80.21 \\
        $m_2\rightarrow m_1$ & 51.21 & 79.83 & 80.17 \\
        $m_2\rightarrow m_5$ & 61.28 & 82.64 & 83.51 \\
        $m_5\rightarrow m_1$ & 44.83 & 77.59 & 81.38 \\
        $m_5\rightarrow m_2$ & 56.08 & 83.60 & 83.77 \\
        \hline
    \end{tabular}
\end{table}

表 \ref{tab:ch6_single_sourceonly_proposed_scene} 进一步显示，TPU-\allowbreak{}Deep\allowbreak{}JDOT 在 6 个单源迁移任务上均明显高于 SourceOnly。其中，$m_5\rightarrow m_1$ 和 $m_5\rightarrow m_2$ 的提升分别达到 32.76 和 27.51 个百分点，说明当源目标工况之间存在较强偏移时，非平衡传输能够减少小批量中错误强制匹配，源域类别原型和时序统计代价也能够帮助模型保留故障类别结构。因此，第三章提出的时序原型非平衡联合分布对齐方法在单源无监督跨工况故障诊断中具有明确有效性。

\subsection{多证据置信均衡伪标签有效性}

第四章提出的 CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 在第三章方法基础上进一步利用目标域无标签样本。该方法通过冻结 TPU 教师锚点、传输语义、教师预测语义和源域原型语义共同筛选伪标签，并结合动态阈值、类别均衡、强弱增强一致性约束和教师--学生预测融合，使目标域训练样本的利用从单纯对齐扩展为高置信目标样本驱动。

从表 \ref{tab:ch6_single_sourceonly_proposed_average} 可以看出，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 的平均准确率达到 82.02\%，较 SourceOnly 提升 26.14 个百分点；宏平均 F1 和平衡准确率分别提升 30.76 和 26.29 个百分点。相较于 SourceOnly，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 在所有单源迁移任务上均取得明显更高的目标域准确率，尤其在 $m_5\rightarrow m_1$、$m_1\rightarrow m_2$、$m_2\rightarrow m_1$ 和 $m_5\rightarrow m_2$ 等任务中，目标域诊断性能提升较为充分。

与第三章方法相比，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 的平均准确率进一步提升 1.48 个百分点，宏平均 F1 和平衡准确率分别提升 1.91 和 1.48 个百分点。逐场景结果显示，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 在 6 个单源迁移任务上均高于 TPU-\allowbreak{}Deep\allowbreak{}JDOT，其中 $m_1\rightarrow m_2$ 和 $m_5\rightarrow m_1$ 的额外提升分别达到 3.00 和 3.79 个百分点。这说明冻结教师锚点、多证据伪标签筛选和教师--学生预测融合能够在已有对齐基础上继续利用目标域无标签信息，进一步强化目标域决策边界。

\subsection{类条件源域可靠性加权有效性}

第五章提出的 CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 面向多源跨工况诊断任务。与 SourceOnly 直接合并多个源域训练模型不同，该方法为每个源域和每个故障类别估计可靠性，并将源域整体权重细化为源域--故障类别级别的加权关系，从而缓解多源负迁移。

\begin{table}[htbp]
    \centering
    \caption{多源所提方法相对SourceOnly的平均诊断性能}
    \label{tab:ch6_multi_sourceonly_proposed_average}
    \small
    \begin{tabular}{c l c c c}
        \hline
        设置 & 方法 & 准确率 & 宏平均F1 & 平衡准确率 \\
        \hline
        两源 & SourceOnly & 64.83$\pm$4.27 & 60.24$\pm$4.89 & 64.95$\pm$4.38 \\
        两源 & CA-\allowbreak{}CCSR-\allowbreak{}WJDOT & 79.87$\pm$4.58 & 79.68$\pm$4.86 & 79.83$\pm$4.58 \\
        五源 & SourceOnly & 71.33$\pm$13.36 & 66.96$\pm$15.24 & 71.16$\pm$13.60 \\
        五源 & CA-\allowbreak{}CCSR-\allowbreak{}WJDOT & 80.00$\pm$6.00 & 78.52$\pm$6.05 & 79.81$\pm$6.21 \\
        \hline
    \end{tabular}
\end{table}

表 \ref{tab:ch6_multi_sourceonly_proposed_average} 显示，在两源设置中，CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 的平均准确率为 79.87\%，较 SourceOnly 提升 15.04 个百分点；宏平均 F1 和平衡准确率分别提升 19.44 和 14.88 个百分点。在五源设置中，该方法平均准确率达到 80.00\%，较 SourceOnly 提升 8.67 个百分点，宏平均 F1 和平衡准确率也分别提升 11.56 和 8.65 个百分点。这说明，简单增加源域数量并不足以自动获得可靠迁移，而类条件源域可靠性建模能够更有效地利用多源信息。

\begin{table}[htbp]
    \centering
    \caption{多源所提方法相对SourceOnly的逐场景目标域准确率}
    \label{tab:ch6_multi_sourceonly_proposed_scene}
    \footnotesize
    \setlength{\tabcolsep}{3pt}
    \begin{tabular}{c c c c}
        \hline
        迁移任务或目标域 & SourceOnly & CA-CCSR & 平均提升 \\
        \hline
        $m_1+m_2\rightarrow m_5$ & 68.69$\pm$0.65 & 83.56$\pm$0.64 & +14.87 \\
        $m_2+m_5\rightarrow m_1$ & 59.08$\pm$0.29 & 73.56$\pm$1.51 & +14.48 \\
        $m_1+m_5\rightarrow m_2$ & 66.73$\pm$1.58 & 82.48$\pm$0.36 & +15.75 \\
        \hline
        五源$\rightarrow m_1$ & 78.45$\pm$0.00 & 87.47$\pm$0.78 & +9.02 \\
        五源$\rightarrow m_2$ & 64.96$\pm$1.16 & 77.13$\pm$0.17 & +12.17 \\
        五源$\rightarrow m_3$ & 86.59$\pm$0.33 & 86.82$\pm$1.14 & +0.23 \\
        五源$\rightarrow m_4$ & 45.36$\pm$1.73 & 71.69$\pm$1.76 & +26.33 \\
        五源$\rightarrow m_5$ & 78.99$\pm$0.00 & 75.93$\pm$3.72 & -3.06 \\
        五源$\rightarrow m_6$ & 73.63$\pm$2.04 & 80.94$\pm$0.33 & +7.31 \\
        \hline
    \end{tabular}
\end{table}

由表 \ref{tab:ch6_multi_sourceonly_proposed_scene} 可见，CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 在三个两源任务上均显著优于 SourceOnly，说明在小规模多源条件下，类条件可靠性矩阵能够稳定识别不同源域的有效贡献。在五源设置中，该方法在 $m_1$、$m_2$、$m_3$、$m_4$ 和 $m_6$ 目标域上均高于 SourceOnly，尤其在目标域 $m_4$ 上提升 26.33 个百分点，说明类条件选择性迁移对困难目标域具有较强价值。目标域为 $m_5$ 时 SourceOnly 已经较高，额外对齐出现轻微下降，提示多源可靠性估计仍需继续抑制局部负迁移。总体而言，第五章方法在平均性能和多数目标域上均明显优于 SourceOnly，验证了类条件源域可靠性加权的有效性。

\section{方法间综合对比与可视化分析}
\subsection{诊断准确率汇总热力图分析}

本小节在 6.3 节 SourceOnly 对比的基础上，进一步比较各类方法之间的整体差异。单源方法和多源方法分别汇总，避免将单源迁移任务与多源迁移任务混合解释。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{figs/ch6_single_source_heatmap.pdf}
    \caption{单源跨工况实验各方法逐场景准确率汇总热力图}
    \label{fig:single_source_heatmap}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{figs/ch6_single_source_method_mean_accuracy.pdf}
    \caption{单源跨工况实验各方法平均准确率柱状图}
    \label{fig:single_source_mean_accuracy}
\end{figure}

图 \ref{fig:single_source_heatmap} 展示了单源任务中不同方法在各迁移方向上的目标域准确率，图 \ref{fig:single_source_mean_accuracy} 则从方法平均准确率角度概括整体差异。SourceOnly 在多数任务上明显偏低，说明跨生产模式直接泛化能力不足；DeepJDOT、CoDATS 等领域自适应方法整体改善了目标域性能；TPU-\allowbreak{}Deep\allowbreak{}JDOT 在所有迁移方向上高于 DeepJDOT，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 又在所有迁移方向上进一步高于 TPU-\allowbreak{}Deep\allowbreak{}JDOT，表明时序原型非平衡对齐和多证据伪标签机制能够稳定提升单源跨工况诊断效果。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c c}
        ...
        \hline
        SourceOnly & 55.88$\pm$7.67 & 49.97$\pm$7.96 & 55.83$\pm$7.79 \\
        DSAN & 62.42$\pm$13.35 & 58.45$\pm$15.51 & 62.41$\pm$13.67 \\
        CDAN-TS & 67.76$\pm$9.99 & 64.25$\pm$11.51 & 67.61$\pm$9.91 \\
        CoDATS & 74.30$\pm$4.62 & 72.34$\pm$5.72 & 74.33$\pm$4.71 \\
        DeepJDOT & 68.88$\pm$1.76 & 66.04$\pm$1.46 & 69.08$\pm$1.62 \\
        TPU-\allowbreak{}Deep\allowbreak{}JDOT & 80.54$\pm$2.20 & 78.82$\pm$2.18 & 80.64$\pm$2.28 \\
        CBTPU-\allowbreak{}Deep\allowbreak{}JDOT & 82.02$\pm$1.64 & 80.73$\pm$1.54 & 82.12$\pm$1.71 \\
        目标域监督参考 & 90.92$\pm$1.42 & 90.67$\pm$1.66 & 90.98$\pm$1.41 \\
        \hline
    \end{tabular}
\end{table}

表 \ref{tab:single_average_results} 给出了单源方法平均性能。CoDATS 取得 74.30\% 的平均准确率，是传统对比方法中较强的时间序列领域自适应基线；DeepJDOT 的平均准确率为 68.88\%，说明普通联合分布最优传输在部分跨工况方向上仍会受到错误匹配影响。TPU-\allowbreak{}Deep\allowbreak{}JDOT 与 CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 分别达到 80.54\% 和 82.02\%，其中 CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 较最佳传统无监督基线 CoDATS 提升 7.72 个百分点，较 DeepJDOT 提升 13.14 个百分点。与目标域监督参考相比，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 仍存在约 8.90 个百分点差距，说明目标域缺少标签仍是跨工况诊断的核心困难。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{c c c c c c}
        \hline
        迁移任务 & SourceOnly & DeepJDOT & TPU-DJDOT & CBTPU-DJDOT & 监督参考 \\
        \hline
        $m_1\rightarrow m_2$ & 55.03 & 66.67 & 80.07 & 83.07 & 91.71 \\
        $m_1\rightarrow m_5$ & 66.84 & 70.31 & 79.51 & 80.21 & 89.58 \\
        $m_2\rightarrow m_1$ & 51.21 & 71.21 & 79.83 & 80.17 & 92.24 \\
        $m_2\rightarrow m_5$ & 61.28 & 68.58 & 82.64 & 83.51 & 88.72 \\
        $m_5\rightarrow m_1$ & 44.83 & 69.31 & 77.59 & 81.38 & 91.55 \\
        $m_5\rightarrow m_2$ & 56.08 & 67.20 & 83.60 & 83.77 & 91.71 \\
        \hline
    \end{tabular}
\end{table}

从表 \ref{tab:single_scene_accuracy} 看，TPU-\allowbreak{}Deep\allowbreak{}JDOT 在 6 个迁移方向上均高于 DeepJDOT，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 又在 6 个迁移方向上均高于 TPU-\allowbreak{}Deep\allowbreak{}JDOT 和其他无监督对比方法，说明原型约束、时序统计代价、冻结教师锚点和伪标签筛选可以共同改善目标域决策边界。需要注意的是，不同场景中的领先幅度并不相同：例如 $m_2\rightarrow m_5$ 中 CoDATS 已达到 81.77\%，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 的优势为 1.74 个百分点；$m_5\rightarrow m_2$ 中 DSAN 达到 81.13\%，CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 的优势为 2.65 个百分点。该结果说明，部分传统方法在特定迁移方向上仍具有竞争力，但冻结单源主线下本文方法的总体排序更加稳定。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{figs/ch6_multi_source_heatmap.pdf}
    \caption{多源跨工况实验各方法逐场景准确率汇总热力图}
    \label{fig:multi_source_heatmap}
\end{figure}

图 \ref{fig:multi_source_heatmap} 展示了多源任务中的方法差异。多源 SourceOnly 相比单源 SourceOnly 具有更高平均覆盖能力，但不同目标域之间仍存在明显波动。WJDOT 在部分目标域上优于 SourceOnly，但受全局源域权重限制，稳定性不足；CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 通过源域--类别级可靠性加权改善了多数多源场景，尤其对 WJDOT 的提升更为稳定。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c c}
        ...
        方法 & 准确率 & 宏平均F1 & 平衡准确率 \\
        \hline
        SourceOnly & 64.83$\pm$4.27 & 60.24$\pm$4.89 & 64.95$\pm$4.38 \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{c c c c c c}
        \hline
        迁移任务 & SourceOnly & CoDATS & WJDOT & CA-CCSR & 监督参考 \\
        \hline
...
表 \ref{tab:two_source_average_results} 和表 \ref{tab:two_source_scene_accuracy} 表明，在两源设置下，CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 平均准确率高于 CoDATS 和 WJDOT，并在三个目标域上均取得所比较无监督方法中的较好结果。该结果说明，类条件源域可靠性加权在小规模多源场景中能够有效细化源域贡献，避免简单合并源域或全局加权造成的信息混淆。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c c}
        ...
        方法 & 准确率 & 宏平均F1 & 平衡准确率 \\
        \hline
        SourceOnly & 71.33$\pm$13.36 & 66.96$\pm$15.24 & 71.16$\pm$13.60 \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{c c c c c c}
        \hline
        目标域 & SourceOnly & CoDATS & WJDOT & CA-CCSR & 监督参考 \\
        \hline
...
表 \ref{tab:five_source_average_results} 和表 \ref{tab:five_source_scene_accuracy} 显示，五源设置下 CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 明显优于 WJDOT，但略低于 CoDATS。该现象说明，源域数量增加后全局最优传输权重更容易受源域间差异影响，而类条件可靠性加权能够缓解这类问题；与此同时，稳定的领域对抗表征在五源场景中仍具有较强竞争力。目标域为 $m_5$ 时 SourceOnly 已经达到较高准确率，额外对齐没有带来收益，说明多源迁移仍需警惕局部负迁移。

\subsection{故障类别混淆矩阵集中分析}

混淆矩阵用于观察不同方法在类别级别上的错误模式。本小节分别选取单源代表任务 $m_5\rightarrow m_2$ 和两源代表任务 $m_1+m_5\rightarrow m_2$，集中展示核心方法的目标域混淆矩阵。两个代表任务均以 $m_2$ 为目标域，能够反映困难目标域下模型对故障类别边界的改善情况。
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_single_tpu_deepjdot_5_to_2_confusion.pdf}
        \caption{TPU-DJDOT}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_single_cbtpu_deepjdot_5_to_2_confusion.pdf}
        \caption{CBTPU-DJDOT}
    \end{subfigure}
    \caption{单源任务 $m_5\rightarrow m_2$ 中核心方法的目标域混淆矩阵}
    \label{fig:single_confusion_matrix_grid}
\end{figure}

图 \ref{fig:single_confusion_matrix_grid} 中，SourceOnly 的错误分布更分散，说明仅依赖源域监督训练难以处理目标域工况差异。DeepJDOT 通过联合分布最优传输增强了对角线响应；TPU-\allowbreak{}Deep\allowbreak{}JDOT 进一步利用非平衡传输和原型约束减少错误匹配；CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 在多数类别上保持较清晰的对角线结构，说明多证据伪标签机制能够继续强化目标域类别边界。

\begin{figure}[htbp]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_multi_sourceonly_1_5_to_2_confusion.pdf}
        \caption{SourceOnly}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_multi_codats_1_5_to_2_confusion.pdf}
        \caption{CoDATS}
    \end{subfigure}
    \vspace{0.4em}
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_multi_wjdot_1_5_to_2_confusion.pdf}
        \caption{WJDOT}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_multi_ca_ccsr_wjdot_1_5_to_2_confusion.pdf}
        \caption{CA-CCSR-WJDOT}
    \end{subfigure}
    \caption{多源任务 $m_1+m_5\rightarrow m_2$ 中核心方法的目标域混淆矩阵}
    \label{fig:multi_confusion_matrix_grid}
\end{figure}

图 \ref{fig:multi_confusion_matrix_grid} 表明，多源 SourceOnly 虽然利用了两个源域，但仍存在较多类别混淆。CoDATS 的领域对抗表征增强了整体类别识别能力；WJDOT 引入多源联合分布最优传输后，部分类别仍受全局源域权重限制；CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 的混淆矩阵对角线更加集中，说明类条件源域可靠性加权能够减少不可靠源域对特定故障类别的干扰。

\subsection{域融合t-SNE可视化分析}

域融合 t-SNE 可视化用于观察源域和目标域样本在特征空间中的混合程度。本小节同样分开展示单源任务和多源任务，重点关注目标域样本是否从独立分布逐渐靠近源域可迁移表征区域。
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_single_tpu_deepjdot_5_to_2_tsne_domain.pdf}
        \caption{TPU-DJDOT}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_single_cbtpu_deepjdot_5_to_2_tsne_domain.pdf}
        \caption{CBTPU-DJDOT}
    \end{subfigure}
    \caption{单源任务 $m_5\rightarrow m_2$ 中核心方法的域融合t-SNE可视化}
    \label{fig:single_tsne_domain_grid}
\end{figure}

图 \ref{fig:single_tsne_domain_grid} 展示了单源任务中的域分布变化。SourceOnly 特征中源域与目标域仍存在明显分离；DeepJDOT 通过联合分布对齐拉近源目标样本；TPU-\allowbreak{}Deep\allowbreak{}JDOT 在保留类别结构的同时增强域间混合；CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 进一步利用高置信目标样本，使目标域样本更集中地进入可判别区域。

\begin{figure}[htbp]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_multi_sourceonly_1_5_to_2_tsne_domain.pdf}
        \caption{SourceOnly}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_multi_codats_1_5_to_2_tsne_domain.pdf}
        \caption{CoDATS}
    \end{subfigure}
    \vspace{0.4em}
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_multi_wjdot_1_5_to_2_tsne_domain.pdf}
        \caption{WJDOT}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/ch6_multi_ca_ccsr_wjdot_1_5_to_2_tsne_domain.pdf}
        \caption{CA-CCSR-WJDOT}
    \end{subfigure}
    \caption{多源任务 $m_1+m_5\rightarrow m_2$ 中核心方法的域融合t-SNE可视化}
    \label{fig:multi_tsne_domain_grid}
\end{figure}

图 \ref{fig:multi_tsne_domain_grid} 说明，多源场景中域融合不能简单理解为所有源域和目标域完全重合。较合理的结果是目标域样本靠近对其类别可靠的源域特征区域，同时保留不同源域的必要结构差异。CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 相较 WJDOT 更强调源域--类别级选择性迁移，因此其域融合结果更适合解释为“有选择地靠近可靠源域”，而不是无差别地混合所有源域。

\subsection{主线目标达成分析}

\noindent\textbf{单源主线目标}

单源实验的主线目标是：在目标域无标签条件下，缓解单一源域到目标域的工况分布偏移，并提高目标域故障诊断性能。实验结果表明，SourceOnly 的平均准确率仅为 55.88\%，而 TPU-\allowbreak{}Deep\allowbreak{}JDOT 和 CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 分别达到 80.54\% 和 82.02\%。混淆矩阵和域融合 t-SNE 也显示，本文单源方法能够增强目标域类别边界和源目标特征融合。因此，第三章和第四章围绕“更可靠的单源对齐”和“更可靠的目标域无标签样本利用”所设定的目标基本实现。

\noindent\textbf{多源主线目标}

多源实验的主线目标是：在多个源域同时可用时，避免简单合并源域造成的负迁移，并刻画不同源域对不同故障类别的贡献。实验结果表明，CA-\allowbreak{}CCSR-\allowbreak{}WJDOT 在两源设置下平均准确率达到 79.87\%，高于 SourceOnly、CoDATS 和 WJDOT；在五源设置下达到 80.00\%，明显高于 WJDOT，并在多数目标域上优于 SourceOnly。混淆矩阵和域融合 t-SNE 进一步表明，类条件源域可靠性加权能够减少不可靠源域对类别判别的干扰。因此，第五章围绕“多源选择性迁移”和“源域--类别级可靠性解释”的目标基本实现。

总体来看，本文围绕单源和多源跨工况故障诊断提出的三类改良方法均在 SourceOnly 基础上取得明显性能提升，并在方法间比较中展现出各自优势。单源方法主要解决源目标对齐和目标域伪标签利用问题，多源方法主要解决源域可靠性差异和负迁移问题；二者共同支撑了本文“在目标域缺少标签条件下提升工业过程跨工况故障诊断能力”的主线目标。

\section{本章小结}

本章围绕田纳西伊斯曼过程领域自适应数据集，设计了单源和多源跨工况故障诊断实验，并对本文提出的三类方法进行了验证。单源实验表明，源域直接迁移方法性能不足，说明不同生产模式之间存在明显分布偏移；TPU-\allowbreak{}Deep\allowbreak{}JDOT 相比 DeepJDOT 取得更高平均准确率、宏平均 F1 和平衡准确率，验证了时序原型非平衡联合分布对齐的有效性；CBTPU-\allowbreak{}Deep\allowbreak{}JDOT 在全部 6 个单源迁移方向上进一步高于 TPU-\allowbreak{}Deep\allowbreak{}JDOT 及其他无监督对比方法，验证了教师锚点、多证据伪标签筛选和教师--学生预测融合的稳定增益。
...
总体而言，本章实验验证了本文方法在无监督跨工况故障诊断中的有效性和局限性。所提方法能够在多个单源和多源任务中提升目标域诊断性能，并提供一定的类别结构和源域可靠性解释；但与目标域监督参考仍存在差距，在困难迁移场景中仍可能出现目标伪语义估计偏差和多源负迁移问题。这些现象也为下一章的总结与未来工作提供了依据。
...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.27\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.19\textwidth}}
    ...
    $\Theta,\bar{\Theta},\Theta_{\mathrm{TPU}}$ & 学生模型、教师模型和 TPU 阶段模型参数集合 & $\Theta=(\theta,\phi)$，$\bar{\Theta}=(\bar{\theta},\bar{\phi})$ \\
    $\xi$ & EMA 教师模型衰减系数 & 仅在教师未冻结时用于更新 $\bar{\Theta}$ \\
