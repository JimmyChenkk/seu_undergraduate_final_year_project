...
\DontPrintSemicolon
\makeatletter
\SetAlFnt{\seut@control@mainlinespread}
\SetAlCapFnt{\seut@control@mainlinespread}
\SetAlCapNameFnt{\seut@control@mainlinespread}
\SetAlgoInsideSkip{smallskip}
\setlength{\interspacetitleruled}{5pt}
\setlength{\interspacealgoruled}{5pt}
\AtBeginEnvironment{algorithm}{\seut@control@mainlinespread}
\setlength{\@fptop}{0pt}
...
\includepdf[pages=-,pagecommand={\thispagestyle{empty}}]{resources/front_cover.pdf}
\includepdf[pages=-,pagecommand={\thispagestyle{empty}}]{resources/claim_sign_page.pdf}
\includepdf[pages=-,pagecommand={\thispagestyle{empty}}]{resources/AI_used_claim.pdf}

\frontmatter
\begin{abstract}[zh]
田纳西伊斯曼过程在不同生产模式下会出现变量相关结构变化和故障响应形态漂移。源工况训练得到的诊断模型直接用于目标工况时，往往同时面对边缘分布偏移、类别结构变形以及目标故障标签不可用等问题。本文以包含 6 个生产模式的田纳西伊斯曼过程领域自适应数据集为对象，将每个生产模式视为一个领域，在目标域标签仅用于最终评价的约束下，研究单源到单目标和多源到单目标两类跨工况故障诊断任务。

单源部分从 DeepJDOT 的小批量匹配问题出发，提出时序原型非平衡联合分布对齐方法。非平衡传输用于放松批内边际约束，减少目标样本与明显不合适源样本之间的强制匹配；源域类别原型、原始时间窗口统计量和源域监督对比学习则分别补充类别中心、故障动态和源域可分性信息。在此基础上，本文提出多证据置信均衡伪标签方法，用传输语义、固定教师模型预测和源域原型语义共同筛选目标样本，并用动态阈值、类别均衡和强弱增强一致性约束控制伪标签噪声。

多源部分构建源域--故障类别可靠性矩阵，不再只给每个源域一个整体权重，而是从原型距离、类条件传输代价、目标预测熵和源域类别性能等信息中估计不同源域对不同故障类别的贡献。实验中，CBTPU-DeepJDOT 在 6 个单源任务上的平均准确率达到 82.02\%，CA-CCSR-WJDOT 在 6 个五源任务上的平均准确率达到 85.50\%。在本文设置的迁移任务中，这两类结果共同指向一个结论：跨生产模式故障诊断需要同时处理样本匹配、目标样本筛选和源域类别贡献分配。
\keywords{工业过程故障诊断,领域自适应,最优传输,伪标签学习,多源迁移}
\end{abstract}

\begin{abstract}[en]
{\setlength{\emergencystretch}{4em}\hbadness=2000
In the Tennessee Eastman process, a change of production mode affects both variable coupling and fault-response dynamics. A diagnostic model trained in one mode therefore cannot be transferred to another mode by assuming an unchanged data distribution. The main difficulties are marginal distribution shift, deformation of class structure, and the lack of target-mode fault labels during training. Using the Tennessee Eastman domain adaptation benchmark with six production modes, this thesis treats each mode as a domain and studies single-source to single-target and multi-source to single-target fault diagnosis. Target labels are reserved for final evaluation only.

For single-source adaptation, the thesis first modifies DeepJDOT from the perspective of mini-batch transport. The proposed temporal-prototype unbalanced joint distribution alignment method relaxes batch-wise marginal constraints and reduces forced matching between target samples and unsuitable source samples. Source-class prototypes, statistics extracted from the original time window, and supervised contrastive learning are incorporated so that the transport cost reflects feature distance, fault dynamics, and class-center relations. A second-stage multi-evidence confidence-balanced pseudo-labeling method is then used to exploit target samples more cautiously. It accepts target pseudo-labels only when transport semantics, fixed-teacher predictions, and source-prototype semantics are sufficiently consistent, with dynamic thresholds, class balancing, and weak-strong consistency used to control pseudo-label noise. For multi-source adaptation, a source-fault reliability matrix is estimated from prototype distance, class-conditional transport cost, target prediction entropy, and source-class performance, allowing source contributions to vary across fault classes. In the experiments, CBTPU-DeepJDOT obtains an average accuracy of 82.02\% on six single-source tasks, while CA-CCSR-WJDOT obtains 85.50\% on six five-source tasks. These results indicate that cross-mode fault diagnosis benefits from jointly considering sample matching, target-sample selection, and class-level source contribution.
\par}
...
流程工业装置在负荷、产品规格或控制策略调整后，过程变量之间的相关关系会随之改变。以田纳西伊斯曼过程为例，不同生产模式下的正常运行分布并不完全重合，故障发生后的响应轨迹也可能在幅值、持续时间和变量联动关系上发生变化。跨工况故障诊断因此不能简单理解为“在另一批数据上测试同一分类器”，而需要同时面对分布偏移、类别语义保持和目标工况缺少故障标签等约束。

工业过程故障诊断通常包括两个层次：先识别系统是否偏离正常状态，再判断具体故障类型。流程工业变量数量多，变量之间又存在明显耦合，故障往往表现为多变量在一段时间窗口内的共同变化，而不是单个测点的孤立异常。数据驱动方法能够从历史运行数据中学习这类隐含模式，在精确机理模型难以完整建立或维护成本较高的场景中具有应用价值。近年来，深度学习方法依托端到端表征能力，已经被用于旋转机械、化工过程和其他工业故障诊断任务\cite{lei2020applications,wang2025comprehensive}。

现有监督诊断模型大多默认训练数据和测试数据服从相近分布。跨生产模式时，这一前提容易失效：源模式中相对清晰的故障簇，进入目标模式后可能整体平移，也可能出现尺度变化或局部重叠；原先可用的分类边界随之变得不稳。另一方面，目标工况故障样本的标注代价通常较高，低频故障和危险工况更难通过重复试验获得充足标签。本文关注的问题是，在目标域标签不可见的条件下，如何同时利用源工况有标签样本和目标工况无标签样本，使诊断模型适应新的生产模式。

领域自适应与这一设定较为吻合。该类方法以源域有标签数据和目标域无标签数据为输入，试图缩小二者在特征空间或联合分布上的差异\cite{pan2009survey,wang2025comprehensive}。在工业过程故障诊断中，不同生产模式可以分别视为不同领域；若各模式下的故障类别集合一致，无监督领域自适应便可以对应“源工况有标签、目标工况无标签”的工程条件。

本文选取田纳西伊斯曼过程领域自适应数据集作为研究对象。该数据集是化工过程控制与故障诊断研究中常用的基准数据，包含多个生产模式下的过程变量序列；已有研究指出，不同模式之间在变量相关结构和 Wasserstein 距离上存在可观差异\cite{montesuma2024benchmark}。围绕该数据集，本文分别研究单源域到单目标域和多源域到单目标域两类任务，并从最优传输、时序原型、置信伪标签和类条件源域可靠性等方面展开方法设计。

\section{国内外研究现状}

\subsection{工业过程故障诊断研究现状}

工业过程故障诊断方法通常可分为基于机理模型、基于信号处理和基于数据驱动三类。机理模型依赖系统方程、残差分析或状态估计，具有较好的物理解释性；但在变量众多、控制回路复杂的流程工业中，精确建模、参数辨识和模型维护均需要较高成本。过程监测领域的综述指出，数据驱动建模已成为复杂流程工业监测的重要路线，其优势在于能够直接利用历史运行数据刻画多变量过程状态\cite{qin2012survey,ge2017review}。

基于信号处理的方法一般先构造时域、频域或时频域特征，再结合分类器完成故障识别。这类方法在旋转机械和传感器信号分析中应用较多，但特征设计往往依赖人工经验，对设备类型和工况变化较敏感。随着工业数据规模扩大，支持向量机、随机森林、人工神经网络、卷积网络等数据驱动模型逐渐被用于化工过程故障检测与诊断\cite{lei2020applications,taqvi2021review,wang2025comprehensive}。

对于田纳西伊斯曼过程这类多变量时间序列，故障信息常分散在多个变量和多个采样时刻中。真正有助于区分类别的，往往是变量组合在时间窗口内形成的动态响应，而不是某个单点幅值是否异常。全卷积网络、循环网络和注意力结构因此被引入时间序列故障诊断，用于提取局部波动、长时依赖和变量间关系。与此同时，相关研究也指出，深度过程故障诊断在跨工况泛化、类别不均衡和目标域标签不足方面仍存在明显挑战\cite{yu2023challenges}。

\subsection{迁移学习研究现状}

迁移学习关注的是如何把已有场景中的知识用于数据不足的新场景。按迁移对象划分，可讨论样本迁移、特征迁移、模型迁移和关系迁移等形式；按源任务与目标任务的关系划分，又可进一步区分同构迁移、异构迁移和领域自适应等问题\cite{pan2009survey,li2022perspective}。

在故障诊断领域，迁移学习常用于跨设备、跨负载和跨工况任务。以跨工况诊断为例，源工况故障样本相对充足，而目标工况可能只有正常数据或少量未标注数据，此时重新训练目标域监督模型并不现实。深度迁移学习将神经网络表征能力与知识迁移机制结合起来，使模型先在源域学习可迁移故障特征，再用于目标域诊断\cite{li2022perspective}。
...
现有领域自适应方法中，基于度量学习的路线通过最大均值差异、相关对齐、条件分布差异或 Wasserstein 距离等度量缩小域间差异。最优传输方法能够保留样本级匹配关系，并从分布几何角度描述源域样本向目标域样本的对应方式\cite{courty2017otda}。DeepJDOT 进一步把最优传输放入深度特征与标签预测的联合空间，使对齐过程不仅考虑特征距离，也考虑分类语义约束\cite{damodaran2018deepjdot}。

另一类常用方法是基于对抗学习的特征对齐。领域对抗网络通过领域判别器促使特征提取器学习源域和目标域难以区分的表示；面向时间序列数据，CoDATS 使用卷积式时间序列特征提取器并结合领域对抗学习，可处理传感器序列跨域迁移任务\cite{wilson2020codats}。在多源场景下，WJDOT 通过逐源联合分布最优传输和源域权重学习，避免把多个源域简单合并为一个大源域\cite{turrisi2022wjdot}。

针对田纳西伊斯曼过程领域自适应数据集的基准研究表明，不同生产模式确实会造成明显分布偏移，基于最优传输的方法在该基准上总体优于部分基于最大均值差异、H-distance 和对抗学习的基线\cite{montesuma2024benchmark}。不过，这些结果也暴露出后续研究空间：DeepJDOT 类方法在小批量训练中仍可能受到强制匹配影响；伪标签若只依赖分类器最大概率，容易把早期错误继续放大；多源方法若只学习源域整体权重，则难以描述某个源域只对部分故障类别可靠的情况。

\section{国内外研究现状总结}

现有研究已经说明，工业故障诊断不再只依赖机理模型或人工特征，深度表征学习和迁移诊断正在成为跨工况问题中的常用工具。对本文使用的 TEP 多生产模式数据而言，单纯换用更复杂的分类器并不能解决全部问题。真正影响目标域性能的，是源目标样本在无标签条件下能否匹配到相近故障语义，目标样本何时可以转化为可信伪监督信号，以及多个源域中哪些模式、哪些故障类别更值得迁移。

在单源场景中，联合分布最优传输能够同时使用特征距离和标签预测信息，但其平衡边际假设与小批量训练并不总是吻合。当一个批次内源域和目标域的类别组成差异较大时，传输计划可能为了满足边际约束而保留低质量匹配。进入伪标签阶段后，分类器最大概率也不能直接等同于正确性，尤其在目标边界尚未稳定时，少量错误标签会被后续训练继续放大。多源场景的问题则更细：增加源域数量可以提供更多样本，却也可能把与目标模式相距较远的类别一起带入训练。

因此，本文把方法设计拆成两条相互衔接的路线。单源路线先处理“怎么对齐”的问题：在 DeepJDOT 基础上引入非平衡传输、源域原型和时序统计代价，尽量让匹配关系避开低质量样本对；随后处理“哪些目标样本能用”的问题，用传输、教师模型和原型三类证据共同筛选伪标签。多源路线则不再把源域看成一个整体权重，而是在 CoDATS 和 WJDOT 的基础上估计源域--故障类别关系，使某个源域可以只在它更擅长的类别上发挥作用。

\section{核心问题与主要创新}

本文的研究目标是在目标工况故障标签不可见的条件下，利用源工况有标签数据完成跨生产模式故障诊断。结合 TEP 数据中 6 个生产模式、29 个状态类别以及多变量时间窗口的特点，本文主要围绕以下三个问题展开。
...
第三，多源跨工况诊断中的源域负迁移问题。多源任务虽然提供了更多有标签样本，但不同源域与目标域之间的接近程度并不一致，同一源域对不同故障类别的作用也可能不同。为此，本文提出类条件源域可靠性加权方法，在 CoDATS 的卷积式时间序列表征和 WJDOT 的逐源联合分布传输之上，加入源域--故障类别可靠性矩阵，使多源迁移可以在类别层面选择源域信息。

对应上述问题，本文的主要创新点如下。

（1）提出基于时序原型非平衡联合分布对齐的单源领域自适应故障诊断方法。该方法利用非平衡最优传输缓解源目标样本错误强制匹配，并通过源域类别原型和时间序列统计代价增强故障结构表达，结合源域监督对比学习提升特征空间类别可分性。

（2）提出基于多证据置信均衡伪标签的单源领域自适应故障诊断方法。该方法融合传输语义、教师预测语义和源域原型语义三类证据，对目标域样本进行一致性筛选，并结合动态置信阈值、类别均衡和强弱增强一致性约束，提高无标签目标样本的使用可靠性。

（3）提出基于类条件源域可靠性加权的多源领域自适应故障诊断方法。该方法构建源域--故障类别可靠性矩阵，使模型能够根据不同源域对不同故障类别的贡献进行选择性迁移，从而缓解多源负迁移并提升结果解释性。

\section{技术路线}

本文技术路线按照数据、模型、单源方法、多源方法和实验验证展开。数据层面，本文使用田纳西伊斯曼过程领域自适应数据集，将 6 个生产模式分别作为领域；每个样本为包含 34 个连续过程变量的时间序列窗口，标签覆盖正常状态和 28 类故障。实验中目标域标签只在最终计算指标时使用，不参与训练、参数选择或模型选择。

模型层面，单源任务以 DeepJDOT 为起点，通过深度特征与标签预测的联合传输刻画源目标样本关系；多源任务以 WJDOT 为起点，保留逐源传输统计而不是把所有源域直接合并。考虑到过程数据是多变量时间序列，本文采用一维卷积式特征提取结构，并在多源方法中加入 CoDATS 风格的领域对抗约束作为表征学习补充。

方法层面，第三章针对 DeepJDOT 的平衡传输约束和故障动态利用不足，引入非平衡传输、源域类别原型、时序统计代价和源域监督对比学习。第四章在第三章模型基础上采用固定教师参考模型和三类语义证据，筛选较可信的目标伪标签。第五章面向多源任务，构造源域--故障类别可靠性矩阵，用类别粒度的权重替代单一源域整体权重。

实验层面，本文设置 6 个单源任务、3 个两源任务和 6 个五源任务，使用准确率、宏平均 F1、平衡准确率、混淆矩阵和 t-SNE 可视化进行分析。除平均性能外，实验还单独比较代表迁移方向和源域--类别权重热力图，避免只凭某一个任务的结果判断方法有效性。

\section{论文结构}

本文共分为七章，各章之间的关系如图 \ref{fig:ch1_thesis_structure} 所示。前两章首先界定跨工况诊断问题，并介绍后续方法所依赖的领域自适应模型基础；第三章至第五章分别对应单源对齐、单源伪标签和多源可靠性三条方法线；第六章再把这些方法放到统一的 TEP 迁移任务中比较；第七章回到方法边界和后续实验设计。

第一章说明为什么 TEP 多生产模式适合用来讨论跨工况诊断，并梳理工业过程故障诊断、迁移学习和领域自适应的相关工作。第二章则更偏向方法准备，介绍跨工况任务设定、联合分布最优传输、DeepJDOT、WJDOT 和 CoDATS，后文的三个方法章都建立在这些模型之上。

第三章和第四章都处理单源到单目标任务，但分工不同。第三章提出基于时序原型非平衡联合分布对齐的单源领域自适应故障诊断方法（TPU-DeepJDOT，Temporal-Prototype Unbalanced Deep Joint Distribution Optimal Transport），先解决源目标样本如何对齐，重点放在非平衡传输、源域类别原型和时间序列统计代价上；第四章提出基于多证据置信均衡伪标签的单源领域自适应故障诊断方法（CBTPU-DeepJDOT，Confidence-Balanced Temporal-Prototype Unbalanced Deep Joint Distribution Optimal Transport with Multi-Evidence Pseudo-Labeling），则接着处理目标样本能不能参与训练的问题，通过传输语义、教师预测语义和源域原型语义筛掉风险较高的伪标签。

第五章转向多源到单目标任务，提出基于类条件源域可靠性加权的多源领域自适应故障诊断方法（CA-CCSR-WJDOT，CoDATS-Augmented Class-Conditional Source Reliability-Weighted Joint Distribution Optimal Transport）。与只学习一个源域整体权重不同，该章把可靠性拆到源域--故障类别层面，并把逐源联合传输、领域对抗约束和教师先验融合放在同一训练框架中。

第六章给出实验协议和结果分析。所有无监督领域自适应方法都遵守同一划分方式，目标域标签只在预测完成后用于评价；实验任务包括 6 个单源任务、3 个两源任务和 6 个五源任务。第七章最后总结三条方法线的结论，也把计算成本、无标签模型选择和真实工业数据适用性等问题单独列出。

\begin{figure}[!htbp]
    % TikZ files are scaled as TeX boxes here. Keep \ignorespaces...\unskip
    % around \input to remove invisible trailing spaces that can shift centering.
    \centering
    \resizebox{\textwidth}{!}{\ignorespaces\input{figs/ch1_thesis_structure.tikz.tex}\unskip}%
    \caption{论文整体框架}
    \label{fig:ch1_thesis_structure}
\end{figure}
...
在这一设定下，最优传输的作用不仅是度量两个经验分布之间的差异，还可以给出样本级跨域匹配关系\cite{courty2017otda}。对多变量时间序列而言，特征提取器还需要保留故障动态响应，因此本章同时介绍卷积式时间序列特征提取和领域对抗学习\cite{wang2017time,fawaz2019deep}。
...
最优传输以传输计划描述一个经验分布向另一个经验分布的质量搬运过程，并以总代价最小为优化目标。在领域自适应中，传输计划可以解释为源域样本与目标域样本之间的软匹配关系。设源域样本集合为 $\{x_i^s\}_{i=1}^{n_s}$，目标域样本集合为 $\{x_j^t\}_{j=1}^{n_t}$，二者之间的代价矩阵为 $C \in \mathbb{R}^{n_s \times n_t}$，其中 $C_{ij}$ 表示源样本 $i$ 与目标样本 $j$ 的匹配代价。最优传输问题可写为
\begin{equation}
    \gamma^\ast
    =
...
\begin{equation}
    ...
    \right\}.
\end{equation}
因此，传输计划 $\gamma$ 不仅反映样本之间的距离关系，也给出了源域样本向目标域样本分配传输质量的方式。

在故障诊断场景中，仅对齐边缘特征分布并不足以保证类别判别性。如果源域某一类故障样本与目标域另一类故障样本在特征空间中被错误匹配，模型可能得到领域不变但类别混淆的表示。联合分布最优传输因此同时考虑特征空间距离和标签预测代价，使传输计划既关注源目标样本在表征空间中的接近程度，也关注源域标签与目标域预测结果之间的一致性\cite{courty2017jdot}。

\subsection{深度联合分布最优传输方法}

深度联合分布最优传输方法（Deep Joint Distribution Optimal Transport，DeepJDOT）是一类面向单源无监督领域自适应的深度最优传输方法\cite{damodaran2018deepjdot}。该方法将深度网络拆分为特征提取器和分类器两部分。为与后文方法记号保持一致，设特征提取器为 $F_\theta(\cdot)$，分类器为 $G_\phi(\cdot)$，则输入样本 $x$ 的深度特征表示为
\begin{equation}
    z = F_\theta(x),
\end{equation}
对应的类别预测概率为
...
\end{equation}
其中 $\operatorname{dist}(\cdot,\cdot)$ 表示特征空间距离，$\ell(\cdot,\cdot)$ 表示标签预测损失，$\lambda_f$ 和 $\lambda_y$ 分别为特征代价和标签代价的权重。第一项描述源域与目标域在深度表征空间中的距离，第二项约束目标域样本预测结果与其匹配源域标签之间的一致性。
...
\begin{equation}
    ...
    \gamma_{ij} C_{ij},
\end{equation}
其中 $\gamma_{ij}$ 表示源域样本 $i$ 与目标域样本 $j$ 之间的传输质量。训练模型时还需保留源域监督分类损失：
\begin{equation}
...
一种直接做法是将所有源域样本合并为统一源域，再执行单源领域自适应。然而，该策略会弱化源域之间的差异。当某些源域与目标域相距较远时，其样本可能干扰目标域诊断并造成负迁移。多源加权联合分布最优传输方法（Weighted Joint Distribution Optimal Transport，WJDOT）通过为不同源域分配权重，在利用多源数据的同时降低不可靠源域的影响\cite{turrisi2022wjdot}。
...
\begin{equation}
    ...
    \alpha_k \geq 0.
\end{equation}
其中，$\alpha_k$ 越大，表示第 $k$ 个源域在目标域适配中占据更大权重；$\alpha_k$ 越小，说明该源域可能与目标域差异较大或贡献较弱。
...
工业过程数据通常表现为多变量时间序列。相比图像数据或普通表格数据，过程序列同时具有时间相关性、动态变化性和变量耦合性。对于田纳西伊斯曼过程，每个样本由多个连续变量在一段时间内的采样值组成，不同故障会在时间维度上呈现不同响应模式。因此，跨工况故障诊断中的特征提取器不仅要提取单个变量的局部变化，还要捕捉不同时间段和不同变量之间的综合故障信息。

CoDATS 是一种面向时间序列传感器数据的卷积式深度领域自适应方法。它通过卷积式时间序列特征提取器学习可迁移表征，并结合领域对抗学习减小源域与目标域之间的特征分布差异\cite{wilson2020codats}。与直接采用图像领域自适应网络不同，CoDATS 面向序列数据设计特征提取结构，因此适合作为本文多源跨工况实验中的对比基线和表征基础。
...
在时间序列分类任务中，全卷积网络常由多个一维卷积块和全局平均池化层组成\cite{wang2017time,fawaz2019deep}。卷积块负责提取局部动态特征，归一化层用于稳定训练过程，非线性激活函数增强模型表达能力，全局平均池化层则将时间维度特征压缩为固定长度向量。设最后一层卷积输出为 $U \in \mathbb{R}^{d_z \times L'}$，全局平均池化可表示为
\begin{equation}
    z_r
    =
    \frac{1}{L'}
...
在本文中，卷积式时间序列特征提取器不仅用于对比方法 CoDATS，也为第五章的多源改良方法提供基础表征。由于最优传输代价依赖样本在特征空间中的距离，特征提取质量会直接影响传输计划的可靠性。若特征表示本身难以区分故障类别，即使后续引入最优传输或源域加权机制，也难以取得稳定适配效果。因此，利用卷积式时间序列网络构建可靠故障表征，是多源跨工况诊断的重要基础。

\subsection{基于领域对抗学习的特征对齐方法}

领域对抗学习是一类典型的深度领域自适应方法。该方法利用领域判别器判断特征来自源域还是目标域，同时训练特征提取器生成使领域判别器难以区分的特征表示。通过这一对抗过程，特征提取器在保持源域分类能力的同时，学习源域和目标域共享的领域不变特征\cite{ganin2016dann}。
...
\end{equation}
领域判别器通过最小化 $\mathcal{L}_{\mathrm{dom}}$ 区分源域和目标域；特征提取器则朝相反方向更新，使两域特征在判别器看来尽量接近。该对抗过程可写为
\begin{equation}
    \min_{F_\theta,G_\phi}
    \max_{D_\omega}
    \mathcal{L}_{\mathrm{src}}
...
\end{equation}
其中 $\lambda_d$ 为领域对抗损失权重。实际训练中通常借助梯度反转思想实现这一目标，使领域判别器倾向于区分源域和目标域，而特征提取器倾向于学习使二者难以区分的共享表示。
...
不过，领域对抗学习也存在局限。该方法主要强调源域和目标域整体特征分布难以区分，但不能保证同一故障类别在不同领域之间正确对齐。若过度追求领域不变性，可能削弱类别判别信息，导致故障类别边界模糊。在多源场景中，不同源域对目标域的贡献并不一致，若领域对抗学习简单合并所有源域，则难以区分有益源域和不可靠源域。此外，领域判别器提供的是整体领域混淆信号，缺少对具体源域和具体故障类别迁移贡献的解释。

第五章便沿着这条思路展开：以 CoDATS 的卷积式时间序列表征和领域对抗约束作为基础，再保留 WJDOT 的逐源传输统计，用类条件可靠性权重刻画源域与故障类别之间的细粒度关系。这样，多源方法不只是让特征“看起来像同一个域”，还可以解释某个源模式主要贡献了哪些故障类别。
...
第二章已经给出 DeepJDOT 的基本形式：源样本和目标样本不是只在特征空间中比较，而是在深度特征与标签预测组成的联合空间中建立传输关系。这样的设计比单纯边缘分布对齐更有利于保留类别语义\cite{courty2017jdot,damodaran2018deepjdot}。不过，把它直接用于 TEP 跨生产模式诊断时，有两个细节不能忽略。

第一个细节来自数据形态。TEP 样本是多变量时间序列，同一故障换到另一个生产模式后，可能不只是特征中心发生平移，变量波动幅度和一阶变化趋势也会一起变化。若传输代价只看深度特征距离和目标预测损失，原始过程窗口中的动态信息就没有被显式利用。第二个细节来自小批量训练。源域批次和目标域批次的类别组成很难每次都刚好对应，平衡最优传输却要求两侧质量完整匹配；当某一类在批内缺失或目标样本暂时没有合适对应项时，传输计划仍可能把它分配给错误源类。

本章的改动围绕这两个细节展开。首先，将平衡传输替换为非平衡小批量传输，允许低可靠匹配质量被削减；其次，维护源域类别原型，用目标样本到各源类原型的相对距离修正传输代价；同时，从原始过程窗口中提取均值、标准差和一阶差分统计量，补充深度特征中可能被压缩掉的时序信息。训练前期还加入源域监督对比学习，使源域特征先形成较清楚的类内聚集和类间分离。为便于后文表述，本文将基于时序原型非平衡联合分布对齐的单源领域自适应故障诊断方法记为 TPU-DeepJDOT（Temporal-Prototype Unbalanced Deep Joint Distribution Optimal Transport）。本章方法不使用目标域标签，目标域标签只在最终实验评价阶段使用。方法整体结构如图 \ref{fig:ch3_tpu_architecture} 所示。

\begin{figure}[!htbp]
    \centering
    \resizebox{\textwidth}{!}{\ignorespaces\input{figs/ch3_tpu_architecture.tikz.tex}\unskip}
...
本章的 TPU-DeepJDOT 主要对 DeepJDOT 做了三处调整。第一，非平衡 Sinkhorn 传输放松了小批量边际约束，使一些明显不合适的匹配不必被硬性保留。第二，源域类别原型被作为类别结构参照，用目标样本相对接近哪个源类来影响传输代价。第三，原始窗口的均值、标准差和一阶差分统计量被转化为时序统计代价，补充故障动态信息。源域预热阶段加入的监督对比学习，则用于先把源域特征空间整理得更可分。

这一步完成后，目标域样本仍主要通过传输代价参与训练，还没有真正成为监督信号。第四章就在这个基础上继续推进：哪些目标样本可以被赋予伪标签，哪些样本应该暂时放过，需要由传输计划、教师模型和源域原型共同判断。

\chapter[基于多证据置信均衡伪标签的单源领域自适应故障诊断方法]{基于多证据置信均衡伪标签的\\单源领域自适应故障诊断方法}
\section{引言}

第三章已经改善了单源对齐，但目标域样本在训练中仍然比较“被动”：它们参与传输代价，却没有作为明确的监督样本使用。训练到中后期后，一部分目标样本的预测会变得稳定，这些样本如果完全不用，目标域边界会停留在较粗的对齐结果上；但如果直接采用分类器最大概率作为伪标签，又容易把早期错误固定下来。TEP 跨模式任务类别数较多，模式2等目标域与其他模式差异也较明显，伪标签一旦偏向少数高置信类别，后续训练很容易继续放大这种偏置。

本章采用更保守的做法：目标样本能否进入伪标签集合，不由学生分类器单独决定。教师模型由同一迁移场景下训练得到的 TPU-DeepJDOT 阶段模型初始化，并在后续训练中保持固定；学生模型同样从该阶段模型出发，只在多证据伪标签目标下做小幅微调。筛选时同时使用三类语义分布：非平衡传输计划给出的传输语义、固定教师模型给出的预测语义，以及源域原型给出的原型语义。只有当这些证据在类别判断上相互接近，且传输语义熵较低时，目标样本才被纳入伪标签学习。为便于后文表述，本文将基于多证据置信均衡伪标签的单源领域自适应故障诊断方法记为 CBTPU-DeepJDOT（Confidence-Balanced Temporal-Prototype Unbalanced Deep Joint Distribution Optimal Transport with Multi-Evidence Pseudo-Labeling）。方法整体结构如图 \ref{fig:ch4_cbtpu_pseudolabel} 所示。

\begin{figure}[!htbp]
    \centering
    \resizebox{\textwidth}{!}{\ignorespaces\input{figs/ch4_cbtpu_pseudolabel.tikz.tex}\unskip}
...
本章的 CBTPU-DeepJDOT 不是重新训练一个完全不同的模型，而是在第三章模型已经完成初步对齐之后，进一步利用目标域中较可信的样本。筛选规则由三类证据共同决定：传输计划说明目标样本主要接收哪些源类质量，固定教师模型保留第三章得到的稳定边界，源域原型则提供类别中心参照。三者一致或差异足够小时，再结合融合置信度和传输熵决定是否接受该样本。对已经接受的伪标签，本章还加入类别频率校正，避免少数高置信类别在微调中继续占据主导。

强弱增强一致性用于检查目标时序样本在扰动下是否仍保持相近预测。最终推理时，学生模型并不会无条件替代教师模型，而是只有在置信度明显更高时才被采用。整个训练和融合过程不使用目标域标签，目标域标签仅在第六章中用于最终性能评价。

\chapter[基于类条件源域可靠性加权的多源领域自适应故障诊断方法]{基于类条件源域可靠性加权的\\多源领域自适应故障诊断方法}
\section{引言}

第三章和第四章只考虑一个源工况到一个目标工况。实际跨生产模式诊断中，源域往往不止一个：除目标模式以外，其余生产模式都可能提供有标签样本。多源数据看起来更充足，但并不天然更可靠。一个源模式可能整体接近目标模式，却只在部分类别上有帮助；另一个源模式整体较远，也可能恰好对某些故障类别提供有用参照。若把多个源域简单合并，模型会同时吸收有益信息和噪声信息，负迁移反而更难定位。

本章因此把源域权重拆到故障类别层面。方法沿用 CoDATS 的时间序列表征和 WJDOT 的逐源传输思路，但不只学习一个源域整体权重，而是在每个源域与目标域之间分别统计类条件传输代价，再结合源目标原型距离、目标预测熵和源域类别召回误差，形成源域--故障类别可靠性矩阵。为便于后文表述，本文将基于类条件源域可靠性加权的多源领域自适应故障诊断方法记为 CA-CCSR-WJDOT（CoDATS-Augmented Class-Conditional Source Reliability-Weighted Joint Distribution Optimal Transport）。训练和模型选择过程中不使用目标域标签，目标域标签仅用于最终评价。方法整体结构如图 \ref{fig:ch5_CA_CCSR_WJDOT} 所示。

\begin{figure}[!htbp]
    \centering
    \resizebox{\textwidth}{!}{\ignorespaces\input{figs/ch5_CA_CCSR_WJDOT.tikz.tex}\unskip}
...
通过这一流程，多个源域的作用被拆到类别层面，而不是在训练开始时就被合并成一个大源域。

\section{本章小结}

本章的 CA-CCSR-WJDOT 把多源负迁移问题拆成了更具体的源域--类别可靠性问题。可靠性矩阵同时使用源目标原型距离、类条件传输代价、目标预测熵和源域类别召回误差，并在每个类别内部转换为源域权重。这样，一个源域可以在某些故障类别上占较大权重，在另一些类别上退到次要位置。

方法中还保留了 CoDATS 风格的时间序列领域对抗表征，并用固定教师先验约束防止学生模型在多源传输中发生过度漂移。最终诊断阶段采用教师先验融合进行无标签置信校正。训练和融合过程均不使用目标域真实标签，目标标签仅用于最终实验评价。
...
本章在田纳西伊斯曼过程领域自适应数据集上验证前文方法。实验先说明数据来源、生产模式、样本组织形式、变量选择和预处理方式，再给出固定第一折的数据划分、目标域无标签训练协议和评价指标。任务设置分为两类：一类是模式1、模式2和模式5之间的单源双向迁移，另一类是两源域和五源域条件下的多源迁移。所有方法在相同协议下比较，并用准确率、宏平均 F1、平衡准确率、混淆矩阵和跨域特征可视化共同分析结果。

除 Target-ref 外，本文所有领域自适应方法在训练、伪标签筛选、超参数选择和模型选择过程中都不使用目标域真实标签；目标域标签只在最终预测完成后用于计算指标。由于不同生产模式之间的偏移程度并不相同，多源场景下源域贡献也可能随故障类别变化，本章不会只报告平均值，还会单独分析困难迁移方向、类别混淆和源域可靠性差异。
...
图 \ref{fig:ch6_tpu_mainline_confusion} 给出 $m_5\rightarrow m_2$ 任务中三种方法的目标域混淆矩阵。SourceOnly 的非对角元素较分散，说明仅依赖源域监督训练难以直接适应目标生产模式；DeepJDOT 缓解了部分误判，但仍有若干类别被吸收到错误预测类别；TPU-DeepJDOT 的对角线响应更集中，说明时序原型非平衡联合分布对齐削弱了低质量强制匹配对类别边界的干扰。该代表场景中，三者准确率分别为 56.08\%、67.20\% 和 83.60\%，形成明显递进关系。
...
混淆矩阵、域 t-SNE 和类 t-SNE 指向同一个现象：TPU-DeepJDOT 相比 DeepJDOT 的收益主要来自类别结构保持。非平衡传输削减低可靠匹配质量，原型代价和时序统计代价则让传输计划不再只依赖瞬时特征距离，从而在目标域无标签条件下减少错误对齐。
...
CBTPU-DeepJDOT 的作用更适合理解为第二阶段修正：TPU-DeepJDOT 已经把源目标结构拉近，伪标签阶段再利用固定教师、传输计划、源域原型和类别均衡机制挑出可信目标样本，对目标域决策边界做小幅但稳定的调整。
...
这组三类图像说明，CA-CCSR-WJDOT 相比多源 SourceOnly 改善的不只是最终分类结果，域间融合和类别聚集结构也同步变清楚了。第五章的基本判断在这里得到支持：多源跨工况诊断不能只看源域数量，还要判断每个源域在具体故障类别上的可靠性。
...
图 \ref{fig:single_source_heatmap} 给出 6 个单源迁移任务上的逐任务准确率热力图。横轴包含 SourceOnly、DSAN、CDAN-TS、CoDATS、DeepJDOT、TPU-DeepJDOT、CBTPU-DeepJDOT 和 Target-ref 共 8 种方法，因此可以观察方法在不同源目标模式组合上的波动，而不只依赖代表场景。SourceOnly 在多数任务中偏低；DSAN、CDAN-TS 和 CoDATS 有一定领域自适应收益；DeepJDOT 在部分任务上继续改善，但平衡传输和类别结构保持仍限制了结果。TPU-DeepJDOT 和 CBTPU-DeepJDOT 的优势在热力图中更加稳定。
...
为验证方法提升是否具有跨迁移场景的一致性，本文采用 Wilcoxon signed-rank test 对相邻方法进行配对检验。这里的配对单位是迁移场景，而不是多随机种子重复实验，因此检验结论表述为“跨迁移场景配对显著性”。为使表格含义更加直观，表 \ref{tab:significance_results} 同时列出相邻方法在对应任务范围内的平均准确率提升，其中 $\uparrow$ 表示左侧改进方法相对右侧基线方法的提升幅度，单位为百分点。为控制表格宽度，表中 TPU-$\sim$ 表示 TPU-DeepJDOT。

\begin{table}[!htbp]
    \centering
    \caption{相邻方法准确率提升与跨迁移场景配对显著性检验结果}
    ...
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.32\textwidth}
        ...
        \midrule
        TPU-DeepJDOT 相对 DeepJDOT & 单源6任务 & $\uparrow$11.66 & 6/6 & 0.015625 \\
        CBTPU-DeepJDOT 相对 TPU-$\sim$ & 单源6任务 & $\uparrow$1.48 & 6/6 & 0.015625 \\
        CA-CCSR-WJDOT 相对 WJDOT & 多源共9任务 & $\uparrow$7.69 & 9/9 & 0.001953 \\
        \bottomrule
    \end{tabular}
\end{table}

表 \ref{tab:significance_results} 表明，三组相邻方法不仅在平均准确率上取得提升，而且在对应迁移场景中均保持正向增益。具体而言，TPU-DeepJDOT 相比 DeepJDOT 平均提升 11.66 个百分点，CBTPU-DeepJDOT 相比 TPU-DeepJDOT 平均继续提升 1.48 个百分点，CA-CCSR-WJDOT 相比 WJDOT 在多源任务上平均提升 7.69 个百分点；三组比较的 Wilcoxon $p$ 值均小于 0.05，说明这些提升在跨迁移场景配对比较下具有统计显著性。

\section{本章小结}

本章把第三章至第五章的方法放到同一套 TEP 跨工况协议下比较。单源实验中，TPU-DeepJDOT 相比 DeepJDOT 明显提高了平均准确率、宏平均 F1 和平衡准确率，说明非平衡传输、原型约束和时序统计代价确实改善了单源对齐质量。CBTPU-DeepJDOT 在 TPU-DeepJDOT 的基础上继续取得 6/6 个任务的无监督第一，说明伪标签阶段虽然增益幅度不如第三章大，但方向稳定。

多源实验中，CA-CCSR-WJDOT 在 3 个两源任务和 6 个五源任务上都取得无监督方法第一；五源任务的平均准确率为 85.50$\pm$2.50\%，高于 CoDATS 和 WJDOT。这个结果也解释了为什么只学习源域整体权重还不够：有些源域不是整体可靠或整体不可靠，而是只在部分类别上更有参考价值。

本章没有只保留平均表，而是同时给出逐任务热力图、代表场景混淆矩阵、域 t-SNE、类 t-SNE、源域--类别权重热力图和配对显著性检验。这样做是为了区分两件事：一是方法是否真的提升了目标域诊断性能，二是提升是否来自更合理的跨域结构，而不是某个任务上的偶然数值波动。
...
本文围绕田纳西伊斯曼过程的跨生产模式故障诊断展开。与普通测试集泛化不同，跨生产模式会同时引入分布偏移、目标故障标签不可见和多源负迁移等问题。围绕这些约束，全文形成了三条方法链：单源联合对齐、单源伪标签利用和多源可靠性加权。

第三章对应单源联合对齐。DeepJDOT 能够在特征和标签预测联合空间中建立匹配，但小批量平衡传输会保留部分低质量配对。本文引入非平衡传输以削弱这类匹配，并利用源域类别原型、时间序列统计代价和源域监督对比学习，从类别中心、原始动态和源域可分性三个方面约束对齐过程。

第四章对应单源伪标签利用。第三章完成初步对齐后，目标样本仍不能直接全部纳入监督训练，因为高置信度有时只是错误边界上的过度自信。本文将传输计划、固定教师模型和源域原型作为三类证据；只有它们给出的类别语义足够接近时，目标样本才进入伪标签集合。动态阈值、类别均衡和强弱增强一致性约束则用于控制伪标签偏置。

第五章对应多源可靠性加权。多源设定下，源域数量增加并不必然带来更好结果；一个源域整体接近目标域，也不代表它对 29 个状态类别都同样适合迁移。本文在 WJDOT 的逐源传输基础上构造源域--故障类别可靠性矩阵，把源域贡献从整体权重细化到类别层面，并结合卷积式时间序列表征、领域对抗约束和教师先验保护机制完成多源诊断。

第六章的实验结果与上述方法链基本对应。单源任务中，性能提升主要来自时序原型非平衡联合分布对齐，伪标签阶段在此基础上继续修正目标域边界；多源任务中，类条件源域可靠性加权缓解了简单合并源域带来的负迁移。混淆矩阵、t-SNE 可视化和源域可靠性分析也说明，方法收益不只是平均准确率的变化，还体现在类别结构和源域贡献的可解释变化上。

\section{不足与边界}

本文方法仍存在若干边界。源工况与目标工况差异过大时，源域原型、传输计划和教师预测都会受到偏移影响，目标域语义证据可能同时变得不稳定。非平衡传输和多证据筛选可以降低风险，但不能保证在所有强偏移任务中都获得充分提升。

方法复杂度也是一个现实限制。本文同时使用最优传输、原型代价、对比学习、教师模型、伪标签筛选和源域--类别可靠性估计；与源域直接迁移或简单对抗对齐相比，这些机制提高了表达能力，也增加了计算和调参成本。当前实验主要验证诊断效果，对训练耗时、显存占用和机制裁剪后的效率收益讨论仍不充分。

目标域无标签条件下的模型选择仍然困难。本文训练过程不使用目标域评估标签，只能依靠源域性能、目标预测分布、教师先验等间接信号判断模型状态。当目标域预测发生类别坍缩或局部过置信时，这些无标签指标可能与真实目标域性能不完全一致。

本文实验主要基于田纳西伊斯曼过程领域自适应数据集。该数据集适合分析多模式化工过程跨域诊断，但仍属于仿真基准。若面对真实工业现场中更复杂的噪声、漂移、缺失变量和未知故障类型，本文方法的泛化规律还需要进一步验证。

\section{未来工作}

后续工作宜围绕可验证实验继续推进，而不是仅扩展方法形式。

一是开展少量目标标签的标签预算实验。例如分别给每个故障类别提供 1、3、5 个目标样本，比较本文无监督方法、少样本微调方法和半监督领域自适应方法的性能变化。若多证据伪标签能够稳定利用目标样本，它应当在低标签预算下与少量真实标签形成互补。

二是开展连续目标批次的测试时自适应实验。具体做法是将目标工况样本按时间顺序或批次顺序输入模型，只允许使用当前批次无标签数据更新归一化统计、教师预测或伪标签阈值，再与固定模型比较。该实验能够检验本文方法面对目标模式缓慢漂移时是否仍然有效。

三是进一步验证源域--类别可靠性矩阵的可解释性。可在多源任务中进行 leave-one-source-out、类别遮蔽或源域类别重加权扰动实验，观察矩阵中高权重源域被移除后，对应故障类别性能是否下降更明显。若这种对应关系成立，可靠性矩阵才不只是可视化结果，而是具有可检验的诊断解释价值。

四是补充计算效率和机制裁剪实验。对非平衡传输、原型代价、对比学习、教师先验和可靠性矩阵逐项裁剪，记录准确率、宏平均 F1、训练时间和显存占用。这样可以判断哪些机制是性能提升的主要来源，哪些机制更适合在计算资源受限的工业部署中简化。

五是在更接近真实现场的数据上验证鲁棒性，包括含缺失变量、强噪声、工况连续漂移或未见故障类型的场景。若本文方法要进一步面向实际应用，还需要证明其不只适用于固定基准任务，也能在数据质量和故障集合都不稳定的条件下保持可解释的诊断行为。
...
\begin{acknowledgement}
  ...
  感谢蒋徐颢、蔡闻骁、邹滨阳、陈悦澈等学长和同学在学习与成长过程中给予我的帮助与示范。他们在专业学习、科研视野和个人规划方面的经验分享，使我受益良多。
...
\clearpage
\includepdf[pages=-,pagecommand={\thispagestyle{empty}}]{resources/back_cover.pdf}
