...
本文方法名称按全文统一约定书写。第三章方法记为 TPU-DeepJDOT，第四章方法记为 CBTPU-DeepJDOT，第五章方法记为 CA-CCSR-WJDOT。DeepJDOT、WJDOT、CoDATS 作为基线或基础方法名称保留。
...
图 \ref{fig:ch6_cbtpu_confusion} 展示两种方法在 $m_5\rightarrow m_2$ 任务上的混淆矩阵。CBTPU-DeepJDOT 相比 TPU-DeepJDOT 的代表场景准确率由 83.60\% 提升到 83.77\%，宏平均 F1 由 82.42\% 提升到 82.77\%，平衡准确率由 83.87\% 提升到 84.04\%。该提升幅度小于第 6.3.1 节中 DeepJDOT 到 TPU-DeepJDOT 的跃升，但混淆矩阵中局部非对角误判继续减弱，说明伪标签阶段主要承担细化目标域边界的作用。
...
\begin{figure}[!htbp]
    ...
    \end{subfigure}
    \caption{单源任务 $m_5\rightarrow m_2$ 中 TPU-DeepJDOT 和 CBTPU-DeepJDOT 的目标域混淆矩阵对比}
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{CBTPU-DeepJDOT}
    \end{subfigure}
    \caption{单源任务 $m_5\rightarrow m_2$ 中 TPU-DeepJDOT 和 CBTPU-DeepJDOT 的域 t-SNE 对比}
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{CBTPU-DeepJDOT}
    \end{subfigure}
    \caption{单源任务 $m_5\rightarrow m_2$ 中 TPU-DeepJDOT 和 CBTPU-DeepJDOT 的类 t-SNE 对比}
...
本小节验证第五章方法的核心问题：多源设置中仅增加源域数量并不必然带来可靠迁移，模型还需要识别不同源域对不同故障类别的贡献差异。为突出 CA-CCSR-WJDOT 相对直接多源训练的作用，本小节比较五源 SourceOnly 与 CA-CCSR-WJDOT。完整的 CoDATS、WJDOT 等基线优势在第 6.4.2 节结合热力图和平均性能表讨论。

图 \ref{fig:ch6_caccsr_confusion} 展示五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中 SourceOnly 与 CA-CCSR-WJDOT 的混淆矩阵。SourceOnly 虽然使用了多个源域的有标签样本，但目标域准确率只有 64.02\%，说明简单合并多源数据仍会受到跨工况偏移和源域差异影响。CA-CCSR-WJDOT 的目标域准确率达到 82.89\%，宏平均 F1 和平衡准确率分别达到 81.77\% 和 82.57\%，混淆矩阵中对角线响应明显增强，表明类条件源域可靠性加权能够显著减少目标域误判。
...
\begin{figure}[!htbp]
    ...
    \end{subfigure}
    \caption{五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中 SourceOnly 和 CA-CCSR-WJDOT 的目标域混淆矩阵对比}
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{CA-CCSR-WJDOT}
    \end{subfigure}
    \caption{五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中 SourceOnly 和 CA-CCSR-WJDOT 的域 t-SNE 对比}
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{CA-CCSR-WJDOT}
    \end{subfigure}
    \caption{五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中 SourceOnly 和 CA-CCSR-WJDOT 的类 t-SNE 对比}
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.24\textwidth}
        ...
        Target Only & 90.92$\pm$1.42 & 90.67$\pm$1.66 & 90.98$\pm$1.41 & -- \\
        \bottomrule
    \end{tabular}
\end{table}

从单源热力图和平均表可以得到两点结论。第一，单源方法符合 SourceOnly $<$ DeepJDOT $<$ TPU-DeepJDOT $<$ CBTPU-DeepJDOT 的总体趋势，说明本文方法并非只在单个代表任务上有效。第二，CBTPU-DeepJDOT 在 6/6 任务中均为无监督方法第一，说明多证据置信均衡伪标签学习的增益虽然幅度相对较小，但跨迁移方向具有一致性。

\subsection{多源创新方法相对基线方法的优势分析}

图 \ref{fig:multi_source_heatmap} 给出五源留一域任务的逐目标生产模式准确率热力图。该图用于观察在源域数量增加后，不同方法是否能够稳定适应各个目标生产模式。SourceOnly 说明直接合并多源监督数据仍然不足，CoDATS 作为时间序列领域对抗基线具有较强表现，WJDOT 引入逐源联合分布传输但全局源域权重粒度仍然偏粗。CA-CCSR-WJDOT 在各目标生产模式上均取得无监督方法最高准确率，说明类条件源域可靠性加权能够在多源场景下提供更稳定的选择性迁移。
...
表 \ref{tab:two_source_average_results} 给出两源任务的平均诊断性能。CA-CCSR-WJDOT 在 3 个两源目标域上均取得无监督方法第一，平均准确率达到 81.21\%，分别高于 CoDATS 和 WJDOT 3.99 和 8.80 个百分点。
...
表 \ref{tab:multi_average_results} 给出五源留一域任务的平均诊断性能。CA-CCSR-WJDOT 在 6 个目标生产模式上均取得无监督方法第一，平均准确率达到 85.50\%，高于 CoDATS 的 83.44\% 和 WJDOT 的 78.37\%。宏平均 F1 和平衡准确率也呈现相同趋势，说明该方法的收益并非只来自样本数量较多的类别。
...
为解释多源热力图中的稳定优势，图 \ref{fig:class_source_alpha_heatmap} 展示五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中 CA-CCSR-WJDOT 的源域--类别权重。不同故障类别并不依赖同一个源域，例如部分类别更偏向 $m_1$，部分类别更偏向 $m_4$ 或 $m_6$，也有若干类别呈现多个源域共同提供信息的状态。该结果说明，CA-CCSR-WJDOT 的优势不是简单平均多个源域，而是在“源域--故障类别”层面重新分配迁移贡献。

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{figs/ch6_class_source_alpha_heatmap.pdf}
    \caption{五源任务 $\{m_1,m_3,m_4,m_5,m_6\}\rightarrow m_2$ 中 CA-CCSR-WJDOT 的源域--类别权重热力图}
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.37\textwidth}
        ...
        CBTPU-DeepJDOT vs. TPU-DeepJDOT & 单源6任务 & 6/6 & 0.015625 \\
        CA-CCSR-WJDOT vs. WJDOT & 两源与五源共9任务 & 9/9 & 0.001953 \\
        \bottomrule
    \end{tabular}
\end{table}
...
多源实验表明，CA-CCSR-WJDOT 在两源设置中 3/3 第一，在五源留一域设置中 6/6 第一，并在五源任务上达到 85.50$\pm$2.50\% 的平均准确率，整体优于 CoDATS 和 WJDOT。该结果说明，类条件源域可靠性加权能够缓解 WJDOT 全局源域权重粒度过粗的问题，并提升多源选择性迁移能力。

总体而言，本章按照方法递进关系组织实验叙事，正文保留单源主结果表、单源逐任务热力图、两源和五源多源主结果表、五源逐目标热力图、代表场景混淆矩阵与特征可视化、源域--类别权重热力图以及显著性检验表。实验结果共同说明，本文方法能够在目标域无标签条件下提升工业过程跨工况故障诊断性能，并为源域可靠性差异提供必要解释。
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.42\textwidth}>{\raggedright\arraybackslash}p{0.22\textwidth}@{}}
    ...
    $\mathcal{L}_{\mathrm{TPU}}$、$\mathcal{L}_{\mathrm{CBTPU}}$、$\mathcal{L}_{\mathrm{CA}}$ & 第三章、第四章和第五章方法的总体损失 & 分别对应三类方法 \\
