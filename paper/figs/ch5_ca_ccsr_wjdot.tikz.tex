\begin{tikzpicture}[
    ...
    cell/.style={draw=black!35, line width=0.25pt, minimum width=0.36cm, minimum height=0.27cm, inner sep=0pt}
]
...
\node[anchor=north west] at (0.72,7.43) {多源输入};
...
\node[anchor=north west] at (16.92,7.43) {类条件权重与融合};
...
\node[chip, minimum width=3.20cm] at (18.75,5.20)
    {类条件传输损失\\[-1pt]$\mathcal{L}_{CCSR}$};
\node[chip, minimum width=3.20cm] at (18.75,4.05)
    {教师安全融合\\[-1pt]$p^{final}$};
\begin{scope}[shift={(18.10,3.05)}]
    \node[cell, fill=green!22] at (0.0,0.45) {};
    \node[cell, fill=yellow!18] at (0.36,0.45) {};
    \node[cell, fill=green!12] at (0.72,0.45) {};
    \node[cell, fill=yellow!24] at (1.08,0.45) {};
    \node[cell, fill=orange!18] at (0.0,0.18) {};
    \node[cell, fill=green!24] at (0.36,0.18) {};
    \node[cell, fill=green!12] at (0.72,0.18) {};
    \node[cell, fill=yellow!22] at (1.08,0.18) {};
    \node[cell, fill=yellow!24] at (0.0,-0.09) {};
    \node[cell, fill=yellow!14] at (0.36,-0.09) {};
    \node[cell, fill=green!22] at (0.72,-0.09) {};
    \node[cell, fill=green!10] at (1.08,-0.09) {};
\end{scope}
\node[note, text width=3.45cm] at (18.75,2.45)
    {按故障类别选择源域，预测阶段保守校正};

% 总目标
\draw[redcard] (3.75,0.50) rectangle (20.85,1.35);
\node at (12.30,0.92)
    {$\mathcal{L}_{CA\text{-}CCSR}
    =\lambda_s\mathcal{L}_{src}
    +\lambda_d\mathcal{L}_{dom}
    +\lambda_{ot}\mathcal{L}_{WJDOT}
    +\lambda_{ccsr}\mathcal{L}_{CCSR}
    +\lambda_{tea}\mathcal{L}_{tea}$};
