...
def _resolve_summary_output_dir(results_dir: Path, output_dir: Path | None) -> Path:
    ...
    if comparison_root is None:
        base_dir = results_dir.parent if results_dir.name == "tables" else results_dir
        return base_dir / "figures"
    return comparison_root / "figures"


def _resolve_artifact_output_dir(artifact_path: Path, output_dir: Path | None) -> Path:
